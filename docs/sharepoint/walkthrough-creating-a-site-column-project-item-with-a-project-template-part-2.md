...
|Bileşen|Açıklama|
|---------------|-----------------|
|Sihirbaz uygulama|Bu, <xref:Microsoft.VisualStudio.TemplateWizard.IWizard> arabirimini uygulayan `SiteColumnProjectWizard`adlı bir sınıftır. Bu arabirim, sihirbaz başladığında ve tamamlandığında Visual Studio 'Nun çağırdığı ve sihirbaz çalışırken belirli zamanlarda çağrı yaptığı yöntemleri tanımlar.|
|Sihirbaz kullanıcı arabirimi|Bu, `WizardWindow`adlı WPF tabanlı bir penceredir. Bu pencere, `Page1` ve `Page2`adlı iki kullanıcı denetimini içerir. Bu Kullanıcı denetimleri, sihirbazın iki sayfasını temsil eder.<br /><br /> Bu kılavuzda, sihirbaz uygulamasının <xref:Microsoft.VisualStudio.TemplateWizard.IWizard.RunStarted%2A> yöntemi sihirbaz kullanıcı arabirimini görüntüler.|
|Sihirbaz veri modeli|Bu, sihirbaz kullanıcı arabirimi ve sihirbaz uygulamasının arasında bir katman sağlayan `SiteColumnWizardModel`adlı bir ara sınıftır. Bu örnek, sihirbaz uygulamasının ve sihirbaz Kullanıcı arabiriminin birbirinden soyut olarak yardımcı olması için bu sınıfı kullanır; Bu sınıf, tüm sihirbazların gerekli bir bileşeni değildir.<br /><br /> Bu kılavuzda sihirbaz uygulamasının, sihirbaz kullanıcı arabirimini görüntülediğinde bir `SiteColumnWizardModel` nesnesini sihirbaz penceresine geçirir. Sihirbaz kullanıcı arabirimi bu nesnenin yöntemlerini kullanarak Kullanıcı arabirimindeki denetim değerlerini kaydeder ve giriş site URL 'sinin geçerli olduğunu doğrulama gibi görevleri gerçekleştirir. Kullanıcı Sihirbazı tamamladıktan sonra, sihirbaz uygulaması, kullanıcı ARABIRIMININ son durumunu öğrenmek için `SiteColumnWizardModel` nesnesini kullanır.|
|Proje imzalama Yöneticisi|Bu, her yeni proje örneğinde yeni bir Key. snk dosyası oluşturmak için sihirbaz uygulamasının kullandığı `ProjectSigningManager`adlı bir yardımcı sınıftır.|
...
- Bir WPF projesi. <xref:Microsoft.VisualStudio.TemplateWizard.IWizard> arabirimini uygular ve sihirbaz kullanıcı arabirimini bu projede tanımlayacaksınız.
...
1. [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)], SiteColumnProjectItem çözümünü açın.
...
4. **C# Görsel** düğümü veya **Visual Basic** düğümünü genişletin ve **Windows** düğümünü seçin.
...
3. **C# Görsel** düğümünü veya **Visual Basic** düğümünü genişletin ve ardından **Windows** düğümünü seçin.
...
2. **Proje tasarımcısında**, bir C# görsel proje için **uygulama** sekmesini veya bir Visual Basic projesi için **Derle** sekmesini seçin.
...
4. **Ekle** düğmesinin yanındaki oku seçin ve ardından görüntülenen menüdeki **bağlantı olarak ekle** seçeneğini belirleyin.
     ...
     [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)], kod dosyasını bir bağlantı olarak **SharePointCommands** projesine ekler. Kod dosyası **ProjectTemplateWizard** projesinde bulunur, ancak dosyadaki kod de **SharePointCommands** projesinde derlenir.
...
2. Tasarımcının XAML görünümünde, geçerli XAML 'yi aşağıdaki XAML ile değiştirin. XAML bir başlık, Sihirbaz sayfalarını içeren bir <xref:System.Windows.Controls.Grid> ve pencerenin alt kısmındaki gezinti düğmelerini içeren bir kullanıcı arabirimi tanımlar.
...
5. Dosyanın en üstündeki `using` bildirimleri dışında, bu dosyanın içeriğini aşağıdaki kodla değiştirin.
...
3. Visual Basic bir proje geliştiriyorsanız, `UserControl` öğesinin `x:Class` özniteliğinde `Page1` sınıf adından `ProjectTemplateWizard` ad alanını kaldırın. Bu, XAML 'in ilk satırdır. İşiniz bittiğinde, ilk satır aşağıdaki gibi görünmelidir.

    ```xml
    <UserControl x:Class="Page1"
    ```

4. Aşağıdaki kodla, dosyanın en üstündeki `using` bildirimleri dışında, Sayfa1. xaml dosyasının içeriklerini değiştirin.
...
3. Visual Basic bir proje geliştiriyorsanız, `UserControl` öğesinin `x:Class` özniteliğinde `Page2` sınıf adından `ProjectTemplateWizard` ad alanını kaldırın. Bu, XAML 'in ilk satırdır. İşiniz bittiğinde, ilk satır aşağıdaki gibi görünmelidir.

    ```xml
    <UserControl x:Class="Page2"
    ```

4. Page2. xaml dosyası için arka plan kod dosyasının içeriğini, dosyanın en üstündeki `using` bildirimleri dışında, aşağıdaki kodla değiştirin.

     [!code-vb[SPExtensibility.ProjectItem.SiteColumn#3](../sharepoint/codesnippet/VisualBasic/sitecolumnprojectitem/projecttemplatewizard/page2.xaml.vb#3)]
     [!code-csharp[SPExtensibility.ProjectItem.SiteColumn#3](../sharepoint/codesnippet/CSharp/sitecolumnprojectitem/projecttemplatewizard/page2.xaml.cs#3)]

## <a name="implement-the-wizard"></a>Sihirbazı uygulama
 <xref:Microsoft.VisualStudio.TemplateWizard.IWizard> arabirimini uygulayarak sihirbazın ana işlevlerini tanımlayın. Bu arabirim, sihirbaz başladığında ve tamamlandığında Visual Studio 'Nun çağırdığı ve sihirbaz çalışırken belirli zamanlarda çağrı yaptığı yöntemleri tanımlar.
...
5. **SiteColumnProjectTemplate** düğümü altında ProjectTemplate. csproj veya ProjectTemplate. vbproj dosyasını açın ve bundan sonra aşağıdaki `PropertyGroup` öğesini kaldırın.
...
6. Aşağıdaki `None` öğesini kaldırın.
...
5. **ProjectTemplateWizard** projesi için kısayol menüsünü açın ve sonra ProjectTemplateWizard. dll dosyasını oluşturmak için **Oluştur** ' u seçin.
...
2. Geliştirme bilgisayarınızda ProjectTemplateWizard projesi için yerleşik ProjectTemplateWizard. dll derlemesinin tam *yolunu kullanarak,* aşağıdaki komutu çalıştırın:
...
2. Dosyanın sonuna yakın bir şekilde, aşağıdaki `WizardExtension` öğesini `</TemplateContent>` ve `</VSTemplate>` etiketleri arasına ekleyin. `PublicKeyToken` özniteliğin *belirteç* değerini, önceki yordamda elde ettiğiniz ortak anahtar belirteci ile değiştirin.
...
## <a name="add-replaceable-parameters-to-the-elementsxml-file-in-the-project-template"></a>Proje şablonundaki Elements. xml dosyasına değiştirilebilir parametreler ekleme
 SiteColumnProjectTemplate projesindeki *Elements. xml* dosyasına çeşitli değiştirilebilir parametreler ekleyin. Bu parametreler, daha önce tanımladığınız `SiteColumnProjectWizard` sınıfındaki `RunStarted` yönteminde başlatılır. Bir Kullanıcı bir site sütunu projesi oluşturduğunda, Visual Studio otomatik olarak bu parametreleri yeni projedeki *Elements. xml* dosyasında, sihirbazda belirttikleri değerlerle değiştirir.
...
1. SiteColumnProjectTemplate projesinde, *Elements. xml* dosyasının IÇERIĞINI aşağıdaki XML ile değiştirin.
    ...
    ```
     ...
     Yeni XML, `Name`, `DisplayName`, `Type`ve `Group` özniteliklerinin değerlerini özel değiştirilebilen parametrelere dönüştürür.
...
10. Menü çubuğunda **derleme** > **Oluştur çözüm**' ü seçin ve ardından çözümün hatasız olarak derlendiğinizden emin olun.
...
2. ProjectTemplateWizard projesinde, SiteColumnProjectWizard kod dosyasını açın ve sonra `RunStarted` yöntemindeki ilk kod satırına bir kesme noktası ekleyin.
...
2. **Görsel C#**  düğümü veya **Visual Basic** düğümünü (proje şablonunuzun desteklediği dile bağlı olarak) genişletin, **SharePoint** düğümünü genişletin ve ardından **2010** düğümünü seçin.
...
4. Visual Studio 'nun diğer örneğindeki kodun, `RunStarted` yönteminde daha önce ayarladığınız kesme noktasında durduğunu doğrulayın.
...
8. *Elements. xml* ' in sihirbazda belirttiğiniz değerleri içerdiğini doğrulayın.
