...
- Aşağıdaki yollarla **Sunucu Gezgini** **SharePoint bağlantıları** düğümünü genişleten bir [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] uzantısı oluşturma:
...
3. **Yeni proje** iletişim kutusunda,  **C# Visual** veya **Visual Basic** düğümlerini genişletin ve ardından **genişletilebilirlik**' i seçin.
...
2. **Yeni proje** iletişim kutusunda,  **C# Visual** veya **Visual Basic** düğümlerini genişletin ve ardından **Windows**' u seçin.
...
9. Menü çubuğunda **derleme** > **Oluştur çözüm**' ü seçin ve ardından çözümün hatasız derlendiğinden emin olun.
...
2. WebPartNodeExtension projesinde, **SiteNodeExtension** kod dosyasını açın ve ardından `NodeChildrenRequested` ve `CreateWebPartNodes` yöntemlerinde ilk kod satırlarına bir kesme noktası ekleyin.
...
2. Test için kullanmak istediğiniz SharePoint sitesinin **Sunucu Gezgini**Içindeki **SharePoint bağlantıları** düğümü altında göründüğünü doğrulayın. Listede yoksa, aşağıdaki adımları izleyin:
    ...
    2. **SharePoint bağlantısı ekle** iletişim kutusunda, bağlanmak istediğiniz SharePoint sitesinin URL 'sini girin ve **Tamam** düğmesini seçin.
         ...
         Geliştirme bilgisayarınızda SharePoint sitesini belirtmek için **http://localhost** yazın.
...
4. Visual Studio 'nun diğer örneğindeki kodun, `NodeChildrenRequested` yönteminde daha önce ayarladığınız kesme noktasında durduğunu doğrulayın ve ardından projede hata ayıklamaya devam etmek için **F5** tuşunu seçin.
