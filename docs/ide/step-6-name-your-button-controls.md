...
1. bir yöntemi daha eklemek için, ıde 'nin bir yöntem eklemesini sağlamak üzere **Windows Form Tasarımcısı** **onay kutusu** denetimi için kod dosyasını açın `checkBox1_CheckedChanged()` . Bu yöntem, Kullanıcı onay kutusunu seçtiğinde veya temizlediğinde çağrılır.
