...
@No__t_0 özelliğini ayarlamak için, [veri kaynakları penceresinden](add-new-data-sources.md#data-sources-window) öğeleri bir form üzerine sürükledikten sonra bileşen tepsisindeki `TableAdapterManager` seçin ve ardından **Özellikler** penceresinde `UpdateOrder` özelliğini ayarlayın.
...
@No__t_0 sınıfı .NET türü değil. Bu nedenle, belgelerde arama yapamazsınız. Veri kümesi oluşturma sürecinin bir parçası olarak tasarım zamanında oluşturulur.
