...
Visual Studio 'Yu programlı olarak veya bir komut isteminden yüklediğinizde, aşağıdaki eylemleri gerçekleştirmek üzere yüklemeyi denetlemek veya özelleştirmek için çeşitli komut satırı parametreleri kullanabilirsiniz:
...
Visual Studio 2017 sürüm 15,9 için önyükleyici almak üzere [**Visual Studio önceki sürümler**](https://visualstudio.microsoft.com/vs/older-downloads/) sayfasına gidin ve aşağıdaki önyükleyici dosyalarından birini indirin:
...
Visual Studio 2019 Bootstrapper ' nı Visual Studio [İndirmeleri sayfasından](https://visualstudio.microsoft.com/downloads) veya Visual Studio 'nun seçtiğiniz sürümü ve visual Studio [2019 yayınlar](https://docs.microsoft.com/visualstudio/releases/2019/history#installing-an-earlier-release) sayfasından edinebilirsiniz. Kurulum dosyanız ya da önyükleyici--eşleşecektir veya aşağıdakilerden biriyle benzerdir:
...
[Microsoft Update kataloğuna](https://catalog.update.microsoft.com)giderek yönetici güncelleştirmesini alabilir, yüklemek istediğiniz güncelleştirmeyi arayabilir ve İndir düğmesine basabilirsiniz. Yönetici güncelleştirmeleri, Visual Studio 'Nun bilgisayarda zaten yüklü olduğunu varsayar. Yönetici güncelleştirmelerinin uygulanması, yepyeni bir yükleme başlatmaz.

## <a name="bootstrapper-commands-and-command-line-parameters"></a>Önyükleyici komutları ve komut satırı parametreleri

Ürünü yüklemek veya bir düzeni sürdürmek için programlı olarak Visual Studio önyükleyici çağrılırken, ilk parametre gerçekleştirilecek işlemi açıklayan komuttur (fiil). İki kısa çizgi (--) önekli sonraki isteğe bağlı komut satırı parametreleri, bu işlemin nasıl gerçekleşmesi gerektiğini daha da tanımlar. Tüm Visual Studio komut satırı parametreleri büyük/küçük harfe duyarlıdır ve [komut satırı parametre örnekleri](command-line-parameter-examples.md) sayfasında ek örnekler bulunabilir.
...
| `--force` | **Isteğe bağlı**: Bu parametre, Visual Studio bir işlem kullanımda olsa bile Visual Studio 'yu kapanmaya zorlar. |
| `--installWhileDownloading` | **Isteğe bağlı**: yükleme, güncelleştirme veya değiştirme komutu sırasında bu parametre, Visual Studio 'nun ürünü hem paralel olarak hem de yüklemesine olanak tanır. Bu, varsayılan deneyimdir. |
| `--downloadThenInstall` | **Isteğe bağlı**: bir Install, Update veya Modify komutu sırasında, bu parametre Visual Studio 'yu yüklemeden önce tüm dosyaları indirmeye zorlar. Parametresinden birbirini dışlamalı `--installWhileDownloading` . |
...
| ----------------------- | --------------- |
| `--installerUpdateArgs [optional parameters]` | Bu parametre, yönetici güncelleştirme senaryolarıyla ilgili belirli parametrelerin "doğrudan geçiş dizisi" olarak çalışır. Bu amaçla etkinleştirilen isteğe bağlı parametreler şunlardır: <br/><br/> `--quiet`: Bu, yönetici güncelleştirmeleri için varsayılan deneyimdir ve bu, tamamlanma açısından burada listelenmiştir. <br/> `--passive`: Bu parametre, parametresini geçersiz kılar `--quiet` .  Bu, Kullanıcı arabiriminin etkileşimli olmayan bir şekilde görünmesine neden olur. <br/>`--norestart`: Bu parametrenin veya ile birlikte kullanılması gerekir `--quiet` `--passive` ve gerekli yeniden başlatmaların gecikmesine neden olur. <br/>`--noWeb`: Bu parametre, Visual Studio 'Nun ürüne yönelik güncelleştirmeleri Internet üzerinden denetlemesini engeller. <br/>`--force`: Bu parametre, Visual Studio kullanımda olsa bile Visual Studio 'Yu kapanmaya zorlar. İş kaybına neden olabileceğinden bu parametreyi dikkatli kullanın. Bu parametrenin kullanıcı bağlamında kullanılması gerekir. <br/>`--installWhileDownloading`: Bu parametre, Visual Studio 'Nun hem ürünü hem de paralel olarak indirmesini ve yüklemesini sağlar. Bu, yönetici güncelleştirmeleri için varsayılan deneyimdir ve bu, tamamlanma açısından burada listelenmiştir. <br/>`--downloadThenInstall`: Bu parametre, Visual Studio 'Yu yüklemeden önce tüm dosyaları indirmeye zorlar. Parametresinden birbirini dışlamalı `--installWhileDownloading` . |
