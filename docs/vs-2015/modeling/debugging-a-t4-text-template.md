...
|Tasarım zamanı şablonu işlenirken veya bir çalışma zamanı (önceden işlenmiş) şablonu derlerken "ifade çok karmaşık".<br /><br /> [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] çalışma zamanı şablonu tarafından oluşturulan kodu incelemeye çalışırken çalışmayı durduruyor.|Metin bloğu çok uzun. T4, metin bloklarını her şablon satırı için bir dize sabit değeri olan dize birleştirme ifadesine dönüştürür. Çok uzun metin blokları derleyicinin boyut sınırlarını fazla sürebilir.|Uzun metin bloğunu, şöyle bir ifade bloğu ile bölün:<br /><br /> `<#= "" #>`|
