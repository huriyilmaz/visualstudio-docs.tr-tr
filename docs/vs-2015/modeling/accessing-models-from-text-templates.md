...
- Erişmek istediğiniz DSL için yönerge işlemcileri belirtin. Bu, kendi etki alanı sınıflarını, özelliklerini ve ilişkilerini metin şablonunuzun kodunda kullanabilmeniz için DSL 'niz için derlemeleri yükler. Ayrıca, belirttiğiniz model dosyasını da yükler.
  ...
  `.tt`DSL minimal dil şablonundan yeni bir çözüm oluşturduğunuzda hata ayıklama projesinde aşağıdaki örneğe benzer bir dosya oluşturulur [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] .
...
- Kod parçalarının yazıldığı dil C# ' dir, ancak herhangi bir türde metin oluşturabilirsiniz. Ayrıca, [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] özelliğini yönergesine ekleyerek kodu yazabilirsiniz `language="VB"` `template` .

- Şablonda hata ayıklamak için `debug="true"` `template` yönergeye ekleyin. Bir özel durum oluşursa şablon başka bir örneğinde açılır [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] . Kodda belirli bir noktada hata ayıklayıcıya bölmek istiyorsanız, ifadesini ekleyin `System.Diagnostics.Debugger.Break();`
...
## <a name="loading-models-dynamically"></a>Modelleri dinamik olarak yükleme
 ...
 Daha az olağan bir senaryoda, yalnızca bir dosya adına sahip olduğunuz ve geçerli projede olmayan bir model dosyası açmak isteyebilirsiniz [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] . Bu durumda, [nasıl yapılır: program kodunda dosyadan model açma](../modeling/how-to-open-a-model-from-file-in-program-code.md)bölümünde açıklanan tekniği kullanarak dosyayı açabilirsiniz.
