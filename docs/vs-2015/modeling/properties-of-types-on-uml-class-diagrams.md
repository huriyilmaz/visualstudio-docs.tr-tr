...
> [!NOTE]
> Bu konu, UML sınıf diyagramlarındaki türlerin özellikleri hakkındadır. Daha fazla bilgi edinmek için aşağıdaki kaynaklara bakın:
...
|**Özellik**|**Varsayılanını**|İçinde görünür|Açıklama|
|------------------|-----------------|----------------|-----------------|
...
|**Color**|Tür türü için varsayılan|Tüm öğeler|Bu şeklin rengi. Diğer özelliklerden farklı olarak, bu, temeldeki model öğesinin bir özelliği değildir. Aynı türde farklı görünümlerde farklı renkler olabilir.|
|**Soyut**|Yanlış|Sınıf|True ise, sınıf örneklenemez ve temel sınıf olarak kullanılmak üzere tasarlanmıştır.|
|**Yaprak**|Yanlış|Sınıf, arabirim|True ise, türün türetilmiş türleri olması amaçlanmamıştır.|
|**Etkin**|Yanlış|Sınıf|True ise, bu türün her örneği denetim iş parçacığı ile ilişkilendirilir.|
|**Görünürlük**|Genel|Class, Interface, Enumeration|-Genel-genel olarak görünür.<br />-Private-bu tür, sahibi olan paketin içinde görünür.<br />-Package-paket içinde görünür.|
...
|**Şablon bağlama**|(yok)|Class, Interface, Enumeration|Boş değilse, bu tür parametre değerlerini bu şablon sınıfına bağlayarak tanımlanır. Şablon parametrelerinin bağlamalarını görmek için özelliği genişletin.|
|**Şablon parametreleri**|(yok)|Class, Interface, Enumeration|Boş değilse, burada listelenen parametrelere sahip bir şablon sınıftır. Parametreleri eklemek veya bağımsız parametrelerin özelliklerini görüntülemek için **[...]** düğmesine tıklayın.|
