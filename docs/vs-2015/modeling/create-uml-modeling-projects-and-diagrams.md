...
> [!NOTE]
> Program kodunu görselleştirmek için kullanılan, .NET sınıf diyagramı olan başka bir diyagram türü vardır. Daha fazla bilgi için bkz. [sınıfları ve türleri tasarlama ve görüntüleme](https://msdn.microsoft.com/library/ab7aty24.aspx).

## <a name="create-a-diagram-in-a-modeling-project"></a><a name="CreatingModelingDiagrams"></a> Modelleme projesinde diyagram oluşturma
...
3. **Yeni öğe Ekle-** *\<project name>* Iletişim kutusunda, **Şablonlar**altında, modelleme diyagramı türü ' ne tıklayın, örneğin **UML bileşen diyagramı**.
...
## <a name="removing-modeling-diagrams-from-a-project"></a><a name="RemovingModelingDiagrams"></a> Bir projeden modelleme diyagramlarını kaldırma
...
- **Çözüm Gezgini**' de, diyagramı temsil eden ana dosyaya sağ tıklayın ve ardından **Sil**' e tıklayın.
    ...
    > [!NOTE]
    > Her diyagramda iki dosya bulunur, diğeri diğeri bir yan. Örneğin, adında bir bileşen diyagramınız varsa `CD1` , adlı dosyayı silmelisiniz `CD1.componentdiagram` . Adlı yan kuruluş dosyası `CD1.componentdiagram.layout` otomatik olarak silinir.
...
3. **Varolan öğe Ekle** iletişim kutusunda, diyagram dosyasını bulun, dosyayı seçin ve ardından **Ekle**' ye tıklayın.
    ...
    > [!NOTE]
    > Her diyagramda dosya sisteminde bir dosya çifti bulunur. Uzantıya sahip bir dosya seçmeyin `.layout` . Ayrıca, Visual Studio birden çok modelleme projesine mevcut UML diyagramları eklemeyi desteklemez. Her diyagram dosyası oluşturulduğu modelleme projesi içinde açılmalıdır. Bunun nedeni, UML diyagramının modelleme projesi tarafından sahip olunan bir modelin görünümünü göstereceğinden oluşur.

## <a name="diagrams-that-do-not-require-modeling-projects"></a><a name="NonModelDiagrams"></a> Modelleme projesi gerektirmeyen diyagramlar
...
## <a name="troubleshooting-modeling-projects-and-diagrams"></a><a name="TroubleshootingModelingProjects"></a> Modelleme projelerinde ve diyagramlarda sorun giderme
