...
> [!NOTE]
> Düzenleyici, bir satır içi şemanın veya `xsd:schemaLocation` özniteliği tarafından başvurulan bir şemanın kullanımını doğrulayabilir. Daha fazla bilgi için bkz. [XML belge doğrulaması](../xml-tools/xml-document-validation.md).
...
2. Belge Özellikleri penceresinde, **şemalar** alanındaki düğmesine tıklayın.
    ...
    **XML şemaları** iletişim kutusu görüntülenir. İletişim kutusu, şema önbelleğinde (Catalog. xml dosyasında başvurulan şemalar dahil). xsd uzantılı tüm şemaları ve ayrıca geçerli çözümdeki herhangi bir şemayı, bir `xsd:schemaLocation` özniteliğinde başvurulan ve Visual Studio 'da açık olan veya **Schemas** özelliği.
