 ...
 **Izgara donatıcıları** Bir `Grid` donatıcı, çil üzerinde dikey veya yatay bir çizgi eklenmiş bir üçgen olarak görünür `Grid` . Bir `Grid` donatıcıyı sürüklediğinizde, fareyi taşırken bitişik sütunların veya satırların genişlikleri veya yükseklikleri güncelleştirin.

 `Grid` Donatıcılar, satırlarının ve sütunlarının genişliğini ve yüksekliğini denetlemek için kullanılır `Grid` . Aynı raya tıklayarak yeni bir sütun veya satır ekleyebilirsiniz `Grid` . İki veya daha fazla sütun veya satır içeren bir panel için yeni bir satır veya sütun satırı eklediğinizde `Grid` , daha fazla genişlik ve yükseklik ayarlamanıza olanak tanıyan bir mini araç çubuğu, demiryolu dışında görünür. Mini araç çubuğu, satırlar ve sütunlar için boyutlandırma seçeneklerini ayarlamanıza olanak sağlar `Grid` .
...
## <a name="device-window"></a>Cihaz penceresi
 XAML Tasarımcısı cihaz penceresi, Windows Mağazası veya Windows Phone projeniz için tasarım zamanında çeşitli görünümlerde, ekranlarda ve görüntüleme seçeneklerinde benzetim yapmanızı sağlar. XAML Tasarımcısı çalışırken, **Tasarım** menüsünde cihaz penceresi kullanılabilir. Şu şekilde görünür:
 ...
 **Yüksek karşıtlık** Uygulamanın seçili karşıtlık ayarını temel alarak önizlemesini görüntüleyin. Bu ayar, **varsayılan**dışında bir değer olarak ayarlandığında, `RequestedTheme` app. xaml içinde ayarlanan özelliği geçersiz kılar.
