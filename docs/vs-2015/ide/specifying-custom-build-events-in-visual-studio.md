...
- Görsel C++--[derleme olaylarını belirtme](https://msdn.microsoft.com/library/788a6c18-2dbe-4a49-8cd6-86c1ad7a95cc).
...
- . Bat dosyalarını çalıştıran tüm derleme olaylarından önce `call` bir ifade ekleyin.
...
- Dosya yollarını tırnak işaretleri içine alın.
     ...
     Örnek ([!INCLUDE[win8](../includes/win8-md.md)] için): "% ProgramFiles (x86)% \ Microsoft SDKs\Windows\v8.0A\Bin\NETFX 4,0 Tools\gacbir .exe"-If "$ (TargetPath)"
...
- Gerektiğinde joker karakterleri ekleyin.
    ...
    > [!NOTE]
    > yukarıdaki koddaki `%I` Batch betiklerine `%%I` olmalıdır.
