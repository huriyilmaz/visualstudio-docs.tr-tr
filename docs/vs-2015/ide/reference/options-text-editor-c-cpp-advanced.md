 ...
 **#İnclude otomatik tamamlamayı devre dışı bırak** @No__t_1 deyimlerinin otomatik tamamlamayı devre dışı bırakır.

 **#İnclude otomatik olarak eğik çizgi kullan** "/" Kullanıldığında `#include` deyimlerinin otomatik tamamlanmasını tetikler. Varsayılan sınırlayıcı ' \ ' ters eğik çizgidir. Derleyici her ikisini de kabul edebilir, bu nedenle kod tabanınızı kullanıp kullanmadığını belirtmek için bu seçeneği kullanın.
 ...
 **Üye listesi anahtar sözcüklerini devre dışı bırak** @No__t_1, `class` `switch` gibi dil anahtar sözcükleri üye listesi önerilerinde görünmez.
