...
Türü belirtilmiş veri kümeleri, [TableAdapters](https://msdn.microsoft.com/library/09416de9-134c-4dc7-8262-6c8d81e3f364) ve DataSet sınıflarının ayrı projelere üretilebilmeleri için geliştirilmiştir. Bu, uygulama katmanlarını hızlıca ayırmanızı ve n katmanlı veri uygulamaları oluşturmanıza olanak sağlar.
 ...
 Aşağıdaki yordamda, üretilen kodu içeren projeden ayrı bir projede veri kümesi kodu oluşturmak için Veri Kümesi Tasarımcısı kullanma işlemi açıklanmaktadır `TableAdapter` .

## <a name="separatedatasets-and-tableadapters"></a>Separatedataset ve TableAdapters
 Veri kümesi kodunu koddan ayırdığınızda `TableAdapter` , veri kümesi kodunu içeren proje geçerli çözümde bulunmalıdır. Bu proje geçerli çözümde bulunmuyorsa, **Özellikler** penceresindeki **veri kümesi proje** listesinde kullanılamaz.
...
6. Veri kümesini kaydedin.
   ...
   Varsayılan olarak, veri kümesini ve kodu ayırdıktan sonra `TableAdapter` sonuç, her projedeki ayrı bir sınıf dosyasıdır. Özgün projenin kodu içeren DatasetName. Designer. vb (veya DatasetName.Designer.cs) adlı bir dosyası vardır `TableAdapter` . **DataSet projesi** özelliğinde belirtilen proje, dataset kodunu Içeren DataSetName. DataSet. Designer. vb (veya DatasetName.DataSet.Designer.cs) adlı bir dosyaya sahiptir.
