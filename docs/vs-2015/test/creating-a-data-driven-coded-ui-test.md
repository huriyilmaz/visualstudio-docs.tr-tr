...
5. Testin çalıştığını `AddNumbers()` doğrulamak için yöntemini kullanın. İmleci yukarıda gösterilen test yöntemine yerleştirin, bağlam menüsünü açın ve **Testleri Çalıştır**' ı seçin. (Klavye kısayolu: CTRL + R, T).
...
9. Yöntemi kullanarak testin çalıştığını doğrulayın `ValidateSum()` . İmleci yukarıda gösterilen test yöntemine yerleştirin, bağlam menüsünü açın ve **Testleri Çalıştır**' ı seçin. (Klavye kısayolu: CTRL + R, T).
...
1. Adlı dataDrivenSample projesine bir metin dosyası ekleyin `data.csv` .
...
1. Testi yeniden çalıştırarak testin artık veri odaklı olduğunu doğrulayın.
   ...
   **Rehber**
   ...
   Daha fazla bilgi için bkz [. Visual Studio Ile sürekli teslim Için test etme 2012 – Bölüm 2: birim testi:](https://msdn.microsoft.com/library/jj159340.aspx) [Visual Studio 2012 ile sürekli teslim için, içeri ve test etmeyi test etme – Bölüm 5: Sistem testlerini otomatikleştirme](https://msdn.microsoft.com/library/jj159335.aspx)
