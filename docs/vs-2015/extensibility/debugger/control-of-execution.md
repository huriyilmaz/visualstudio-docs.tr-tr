...
2. IDE, düzenleyicide vurgulanan geçerli kaynak kodu dosyasını ve konumunu belirler.  
  
3. Hata ayıklama oturumu genellikle programın **Continue** metodunu çağırarak bu ilk durdurma olayına yanıt verir.  
  
4. Daha sonra program, bir kesme noktası ile karşılaşana kadar (örneğin, hata ayıklama oturumuna DE bir kesme noktası olayı gönderen) çalışır. Kesme noktası olayı durdurma olayıdır ve bir kullanıcı yanıtı için DE bekler.  
  
5. Kullanıcı bir işlevin içine, üzerine veya dışına geçiş yapmak isterse, IDE, hata ayıklama oturumunun programın `Step` metodunu çağırmasını, adım birimini (yönerge, ekstre veya satır) ve adım türünü (yani, üzerinde mi, yoksa üzerinde mi yoksa dışına mı geçirmeyeceğinizi) ister. Adım tamamlandığında, ve hata ayıklama oturumuna bir DE durdurma olayı olan bir adım Tamam olayı gönderir.  
   ...
   Kullanıcı bir işlevin içine adımla, üstünden veya dışına geçiş yapmak için, IDE, [IDebugProgram2:: Step](../../extensibility/debugger/reference/idebugprogram2-step.md)çağrısını, [stepunit](../../extensibility/debugger/reference/stepunit.md) (yönerge, ekstre veya satır) ve [stepkind](../../extensibility/debugger/reference/stepkind.md)' i çağırmak için şunu ister. Adım tamamlandığında, DE bir durdurma olayı olan SDM 'ye bir [IDebugStepCompleteEvent2](../../extensibility/debugger/reference/idebugstepcompleteevent2.md) arabirimi gönderir.  
