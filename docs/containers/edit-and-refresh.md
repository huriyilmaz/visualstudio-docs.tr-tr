---
title: Yerel bir Docker kapsayıcısı kapsayıcısı içinde uygulama | Microsoft Docs
...
ms.openlocfilehash: 44f2639659380e38f9f60be9a4007ba6be21b38d
ms.sourcegitcommit: 4efdab6a579b31927c42531bb3f7fdd92890e4ac
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 10/26/2021
ms.locfileid: "130351130"
---
# <a name="debug-apps-in-a-local-docker-container"></a>Yerel Docker kapsayıcısı içinde uygulamalarda hata ayıklama

Visual Studio Docker kapsayıcıları geliştirmek ve yerel olarak doğrulamak için tutarlı bir yol sağlar.
...
Bu makalede, yerel bir Docker Visual Studio uygulama başlatmak, değişiklikler yapmak ve ardından değişiklikleri görmek için tarayıcıyı yenilemek için Visual Studio'nin nasıl kullanılası açıklanmıştır. Bu makalede ayrıca kapsayıcılı uygulamalar için hata ayıklama için kesme noktaları ayarlama da açıklanmıştır. Desteklenen proje türleri, .NET Framework .NET Core web ve konsol uygulamalarını içerir. Bu makalede, web uygulamaları ASP.NET Core konsol uygulamaları için .NET Framework kullanırsınız.
...
* Visual Studio Geliştirme iş yükünün yüklü olduğu [2022 RC'ye](https://visualstudio.microsoft.com/downloads) sahip

::: moniker-end

Docker kapsayıcılarını yerel olarak çalıştırmak için yerel bir Docker istemciniz olması gerekir. Daha sonra veya daha yeni bir Windows 10 [Docker Desktop'ı](https://www.docker.com/get-docker)kullanabilirsiniz.
...
Değişiklikleri hızla tekrar etmek için kapsayıcıda uygulama başlatabilirsiniz. Ardından değişiklik yapmaya devam etmek ve bunları diğer dosyalarda olduğu gibi IIS Express.
...
1. Çözüm **Yapılandırması'nın Hata** **Ayıkla olarak ayarlayın.** Ardından **Ctrl** + **F5 tuşlarına** basarak Docker görüntülerinizi yerel olarak çalıştırın.
...
1. Bu Çözüm Gezgini proje düğümüne sağ tıklayın ve Kapsayıcı Düzenleme **Desteği**  >  **Ekle'yi seçin.**  Görüntülenen iletişim kutusunda , öğesini **Docker Compose.** Projenize bir Dockerfile eklenir ve Docker Compose dosyalarıyla birlikte bir proje eklenir.
...
5. Kesme Visual Studio ve değerleri incelemek için Visual Studio'ye geçiş.

   ![Program.cs için kod penceresinin ekran Visual Studio vurgulanmış bir kod çizgisinin sol tarafından ayarlanmış bir kesme noktası.](media/edit-and-refresh/breakpoint-console.png)
...
Kapsayıcınızı el ile değiştirdikten sonra temiz bir kapsayıcı görüntüsüyle yeniden başlatmak için Visual Studio'da Build Clean komutunu kullanın ve  >   ardından normal şekilde derlemeyi deneyin.
