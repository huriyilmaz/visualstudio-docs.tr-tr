...
Bu öğreticide, birden fazla kapsayıcıyı yönetmeyi ve kapsayıcılar arasında iletişim kurarken kapsayıcılar arasında iletişim Visual Studio.  Birden çok kapsayıcının *yönetilmesi için kapsayıcı düzenlemesi* ve Docker Compose, Kubernetes veya Service Fabric. Burada, Docker Compose. Docker Compose, geliştirme döngüsü sırasında yerel hata ayıklama ve test etme için harikadır.
...
1. Projede `WebFrontEnd` Kapsayıcı **Orchestrator Desteği >'ı seçin.** **Docker Destek Seçenekleri iletişim** kutusu görüntülenir.
