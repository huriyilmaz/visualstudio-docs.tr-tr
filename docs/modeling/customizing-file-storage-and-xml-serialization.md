...
description: Visual Studio bir etki alanına özgü dilin (DSL) bir örneğini veya modelini kaydettiğinizde oluşturulan veya güncellenen XML dosyası hakkında bilgi edinin.
...
ms.locfileid: "126634409"
---
# <a name="customize-file-storage-and-xml-serialization"></a>Dosya Depolama ve XML Serileştirmeyi Özelleştirme

kullanıcı, Visual Studio alanına özgü dilin (DSL) bir örneğini veya *modelini* kaydettiğinde, bir XML dosyası oluşturulur veya güncelleştirilir. Dosyayı depoda yeniden oluşturmak için dosya yeniden yüklenebilir.
...
- *Kimlik takma adları* hedef öğenin GUID 'si. Örnek:
...
Ancak, Bunların yerine Kimlik'e Id ile başvurulsa, bilinen adlar aşağıdaki gibi olur:
...
|Gösterimi|Öznitelik ise, özelliği bir xml özniteliği olarak serileştirilmiştir; if Öğesi, bir öğesi olarak seri hale; Yoksay ise, seri hale getirili değildir.|
...
|Öğesi Atla|True ise, kaynak role karşılık gelen XML düğümü şemadan atlanır.<br /><br /> Kaynak ve hedef sınıflar arasında birden fazla ilişki varsa, bu rol düğümü iki ilişkiye ait bağlantılar arasında ayrımlar sağlar. Bu nedenle bu seçeneği bu durumda ayarlamamanizi öneririz.|
