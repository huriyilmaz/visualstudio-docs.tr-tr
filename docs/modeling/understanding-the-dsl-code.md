...
description: Domain-Specific Dili (DSL) çözümünün, Visual Studio'daki DSL örneklerini okumak ve güncelleştirmek için kullanabileceğiniz bir API'yi nasıl Visual Studio.
...
### <a name="the-code-directories"></a>Kod dizinleri
 ...
 \*.tt dosyaları çok az kod oluşturma içerir. Bunun yerine, paylaşılan `<#include>` şablon dosyalarını eklemek için yönergeleri kullanırlar. Paylaşılan dosyalar **\Program Files\Microsoft Visual Studio 10.0\Common7\IDE\Extensions\Microsoft\DSL SDK\DSL Tasarımcısı\11.0\TextTemplates** konumunda bulunabilir
 ...
 *YourDsl*`Schema.xsd`
 ...
 DSL Gezgini'nde seçenekleri ayarerek serileştirmeyi özellerseniz şema uygun şekilde değişir. Ancak, kendi serileştirme kodunuzu yazarsanız, bu dosya artık gerçek şemayı temsil etmeyebilirsiniz. Daha fazla bilgi için [bkz. Dosya Depolama VE XML Serileştirmesini Özelleştirme.](../modeling/customizing-file-storage-and-xml-serialization.md)
 ...
 Her oluşturucu sınıfı DSL Gezgini'nin Bağlantı Oluşturucuları **bölümündeki** bir düğümden oluşturulur. Bir `Connect` yöntem, bir veya daha fazla etki alanı sınıfı çifti arasında ilişkiler oluşturabilir. Her çift, DSL Gezgini'nde oluşturucu düğümü altında Bağlan Bağlantı Oluşturma Yönergesi ile tanımlanır.
 ...
 Belirli bir etki alanı ilişkisi türü oluşturulduğunda çalıştırılan özel kod yazmak için oluşturucu düğümü altında uygun Bağlantı Bağlan Yönergesi'ne tıklayın. Aşağıdaki Özellikler penceresi, Uses **Custom Bağlan**. Çözümü yeniden üretin ve ardından ortaya çıkan hataları düzeltmek için kod girin.

 Kullanıcı bu bağlantı aracını her kullandığında çalışan özel kod yazmak için, bağlantı **oluşturucuslarının Özeldir** özelliğini ayarlayın. Bir kaynak öğeye izin verilip izin verilmediğini, kaynak ve hedefin belirli bir bileşimine izin verilip izin verilmediğini ve bağlantı ederken modelde hangi güncelleştirmelerin yapılacaklarını karara varan kodlar sebilirsiniz. Örneğin, yalnızca diyagramda bir döngü oluşturmazsa bir bağlantıya izin veebilirsiniz. Tek bir ilişki bağlantısı yerine, kaynak ve hedef arasında birkaç ilişkili öğenin daha karmaşık bir desenini örneği olarak kullanabilirsiniz.
 ...
 Bu yönerge işlemcisi, kullanıcılarının DSL'nizin bir örneğini okuyabilen metin şablonları yazmanıza yardımcı olur. Yönerge işlemcisi DSL'niz için derlemeleri (DLS) yükler ve ad alanınız `using` için deyimleri etkili bir şekilde ekler. Bu, metin şablonlarındaki kodun DSL'niz içinde tanımlandığı sınıfları ve ilişkileri kullanmalarını sağlar.

 Daha fazla bilgi için, [bkz. Creating Code from a Domain-Specific Language and](../modeling/generating-code-from-a-domain-specific-language.md) Creating Custom [T4 Text Template Directive Processors](../modeling/creating-custom-t4-text-template-directive-processors.md).
