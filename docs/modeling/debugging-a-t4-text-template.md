...
|-|-|-|
|'{0}' içerme dosyasını yüklemek null veya boş bir dize döndürdü.|Eklenen bir metin şablonu dosyası boşsa gerçekleşir. İleti, eklenen dosyanın dosya adını sağlar.|Include yönergesini kaldırın ya da dosyanın bazı içeriklere sahip olduğundan emin olun.|
...
|Şablon yönergesinde geçersiz bir HostSpecific değeri '{0}' belirtildi. HostSpecific değeri "true" ya da "false" olmalıdır. Varsayılan "false" değeri kullanılacaktır.|`template` yönergesinin ana bilgisayara özgü parametresi hatalı belirtildiğinde gerçekleşir. İleti, şablon yönergesinin satır numarasını sağlar.|Konağa özgü parametreyi "true" veya "false" olarak ayarlayın.|
|' Şablon ' yönergesinde geçersiz bir dil '{0}' belirtildi. Dil "C#" ya da "vb" olmalıdır. Varsayılan "C#" değeri kullanılacaktır.|`template` yönergesinde desteklenmeyen bir dil belirtildiğinde gerçekleşir. Yalnızca "C#" veya "vb" kullanımına izin verilir (büyük/küçük harfe duyarsız). İleti, şablon yönergesinin satır numarasını sağlar.|Şablon yönergesinde `language` parametresini "C#" veya "vb" olarak ayarlayın.|
...
|'{0}' adlı bir yönerge için işlemci belirtilmedi. Yönerge yoksayılacak.|Bir `custom` yönergesi belirtirseniz ancak `processor` özniteliği sağlamazsanız gerçekleşir. İleti, yönergenin adını ve satır numarasını sağlar.|Yönerge için `directive` işlemcisinin adına sahip bir `processor` özniteliği sağlayın.|
|'{1}' adlı yönerge için '{0}' adlı bir işlemci bulunamadı. Yönerge yoksayılacak.|Sistem `custom` yönergesinde belirttiğiniz `directive` işlemciyi bulamadığında oluşur. İleti yönerge adı, işlemci adı ve yönergenin satır numarasını sağlar.|Yönergedeki `processor` özniteliğini yönerge işlemcisinin adına ayarlayın.|
|'{1}' yönergesi için gerekli bir '{0}' parametresi bulunamadı. Yönerge yoksayılacak.|Sistem gerekli yönerge parametresini sağlamıyorsa oluşur. İleti, eksik parametrenin adı, yönerge adı ve satır numarası sağlar.|Eksik parametre sağlayın.|
|'{0}' adlı işlemci '{1}' adlı yönergeyi desteklemiyor. Yönerge yoksayılacak.|Yönerge işlemcisi bir yönergeyi desteklemediği zaman gerçekleşir. İleti, yönerge işlemcisinin adı ile birlikte sorunlu yönergenin adını ve satır numarasını sağlar.|Yönergenin adını düzeltin.|
|'{0}' dosyası için içerme yönergesi sonsuz döngüye neden oluyor.|Döngüsel içerme yönergeleri belirtilmişse görüntülenir (örneğin, A dosyası, dosya A içeren B dosyasını içerir).|Dairesel içerme yönergeleri belirtmeyin.|
