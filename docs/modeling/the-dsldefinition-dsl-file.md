...
ms.locfileid: "112388639"
---
# <a name="the-dsldefinitiondsl-file"></a>DslDefinition.dsl Dosyası

Bu konuda, bir çözümün Dsl projesinde bulunan ve etki alanına özgü bir dili tanımlayan DslDefinition.dsl [!INCLUDE[dsl](../modeling/includes/dsl_md.md)] *dosyasının yapısı açıklanmıştır.* DslDefinition.dsl dosyası etki alanına özgü dilin sınıflarını ve ilişkilerini, etki alanına özgü dilin ve düzenleme araçlarının diyagramı, şekilleri, bağlayıcıları, serileştirme biçimi ve **Araç** Kutusu ile birlikte açıklar. Etki alanına özgü bir dil çözümünde, bu araçları tanımlayan kod DslDefinition.dsl dosyasındaki bilgilere göre oluşturulur.
...
Bu konudaki örnekler Bileşen Diyagramı çözüm şablonundan alındır. Bir örneği görmek için Bileşen Modelleri çözüm şablonunu temel alan etki alanına özgü bir dil çözümü oluşturun. Çözümü oluşturduk sonra DslDefinition.dsl dosyası Domain-Specific Dil Tasarımcısı'nda görünür. Dosyasını kapatın, dosyada sağ tıklayın, **Çözüm Gezgini aç'ın** üzerine **gelin,** **XML Düzenleyicisi'ne** tıklayın ve ardından Tamam'a **tıklayın.**
...
Bu bölüm her türü ve ad alanını tanımlar. Etki alanı özellikleri iki türe sahiptir. `DomainEnumerations` modelde tanımlanır ve DomainModel.cs içinde türler oluşturulur. `ExternalTypes` başka bir yerde (veya gibi) tanımlanan türlere `String` bakın ve hiçbir şey `Int32` oluşturmaz.
...
Bu bölüm, her bağlayıcı aracı için bir bağlantı oluşturucu tanımlar (bağlanacak iki sınıf arasında bağlantı oluşturma aracı). Bu bölüm, bir kaynak ve hedef sınıfa bağlanıp bağlanamayabilirsiniz.

### <a name="diagram"></a>Diyagram

Bu bölüm bir diyagramı tanımlar ve bunu arka plan rengi ve kök sınıfı gibi özellikleri belirtmek için kullanır. (Kök sınıf, diyagram tarafından bir bütün olarak temsil edilen etki alanı sınıfıdır.) Diyagram bölümü, her bir etki alanı sınıfını veya ilişkisini temsil eden şekli veya bağlayıcıyı belirten ShapeMap ve ConnectorMap öğelerini de içerir.

### <a name="designer"></a>Tasarımcı

Bu bölüm bir Araç Kutusu, doğrulama **ayarları,** diyagram ve serileştirme şemasını bir araya getiren bir tasarımcıyı (düzenleyici) tanımlar. Tasarımcı bölümü ayrıca modelin kök sınıfını da tanımlar. Bu sınıf genellikle diyagramın kök sınıfıdır.
...
Bilinen ad sistemi, XML ağacında eşlerin farklı adlara sahip olması gerekir. Bu nedenle, örneğin aynı adla iki sınıfa sahip olan etki alanına özgü bir dil tanımını kaydetmeye çalışsanız doğrulama hataları oluşur. DslDefinition.dsl dosyasını daha sonra doğru şekilde yeniden yükleyebilirsiniz.
...
`NamedElement`, 'den devralınan özelliğine ek olarak kendi özelliklerine sahip olan gibi `Component` `Name` diğer sınıflardan birkaçıdır. `NamedElement` BaseClass alt düğümü bir bilinen ad başvurusu içerir. Başvurulan sınıf aynı ad alanı içinde olduğundan, bilinen ad alanına yalnızca adı gereklidir:
...
- **Ad ve Ad Alanı.** Bu öznitelikler, oluşturulan kodda sınıfının adını ve ad alanını belirtir. Bunların birlikte etki alanına özgü dilde benzersiz olması gerekir.
...
- **HasCustomConstructor**. Bu öznitelik true olarak ayarlanırsa, kendi sürümlerinizi yazabilirsiniz, böylece oluşturucu oluşturulan koddan atlanır.

- **Öznitelikler**. Bu öznitelik, oluşturulan sınıfın CLR Özniteliklerini içerir.

- **BaseClass**. Bir temel sınıf belirtirsiniz, aynı türde olması gerekir. Örneğin, bir etki alanı sınıfının temeli olarak başka bir etki alanı sınıfı ve bölme şekli bir bölme şekline sahip olmalıdır. Temel bir sınıf belirtmezseniz, oluşturulan kodda sınıfı standart bir çerçeve sınıfından türetir. Örneğin, bir etki alanı sınıfı' sınıfından `ModelElement` türetildi.
...
- özniteliği, `PropertyName` diğer uçta yer alan nesnelere erişmek role playing sınıfında kullanılan adı belirtir. Bu ad, ilişkide çapraz geçiş yapmak için şablonda veya özel kodda kullanılır. Örneğin, kaynak `PropertyName` rolün özniteliği olarak `Targets` ayarlanır. Bu nedenle, aşağıdaki kod çalışır:
    ...
    ```
     ...
     Kural gereği, çokluğun ZeroMany veya OneMany olması, özellik adlarının çoğul olmasıdır.
...
- Rolün `Name` adı, İlişki sınıfında bir bağlantının bu sonuna başvurmak için kullanılan addır. Kural gereği, her bağlantının her ucunda yalnızca bir örnek olduğundan rol adı her zaman tekildir. Aşağıdaki kod işe çalışır:

    ```
    Connection connectionLink = ...; OutPort op = connectionLink.Source;
    ```

- Varsayılan olarak, `IsPropertyGenerator` özniteliği true olarak ayarlanır. False olarak ayarlanırsa Rol Oynatıcısı sınıfında hiçbir özellik oluşturulmaz. (Bu `op.Targets` durumda, , örneğin, çalışmaz). Ancak, özel kod ilişkiyi açıkça kullanıyorsa, özel kod kullanarak ilişkiyi çapraz geçiş yapmak veya bağlantıların kendilerine erişim elde etmek yine de mümkündür:
...
Bağlantı oluşturma yolunda birden fazla kesim kullanabilirsiniz. Bu durumda, son segment hangi bağlantının oluşturulacak olduğunu tanımlar. Önceki segmentler üst sınıftan yeni bağlantının oluşturulacak nesneye doğru ilerler.
...
Serileştirilmiş model dosyasında, bir öğenin tam bilinen adı, model kökünden ekleme ilişkileri ağacının aşağı doğru yoludur ve her noktada bilinen anahtarı alıntılar. Örneğin, InPort'lar Bileşenler'in içine katıştırılmıştır ve bunlar da model köküne katıştırılmıştır. Bu nedenle geçerli bir bilinen ad:
...
**UseFullForm** özniteliği true olarak ayarlanırsa, fazladan bir iç içe yerleştirme katmanı ortaya çıktı. Bu katman, ilişkinin kendisini temsil eder. İlişkinin özellikleri varsa özniteliği true olarak ayar gerekir.
...
(Bağlantı İlişkisi, öğesi ve öznitelik adlarını sağlayan kendi XML sınıf verilerine sahip.)

**OmitElement** özniteliği true olarak ayarlanırsa, ilişki rolü adı atlanır. Bu ad serileştirilmiş dosyayı kısaltır ve iki sınıfta birden fazla ilişki yoksa belirsiz olur. Örneğin:
...
- ConnectorHasDecorators, ile arasındaki ekleme `Connector` `Decorator` ilişkisidir. `UseFullForm` , İlişkinin adının Bağlayıcı nesnesinde yer alan her bağlantı için özellik listesiyle birlikte görünür olacak şekilde ayarlanmıştır. Ancak, `OmitElement` içine eklenmiş birden çok `RoleElementName` bağlantının içine girilemayacak şekilde de ayarlanmıştır: `Connector`
...
Şekil ve Bağlayıcı tanımları, aşağıdakilere ek olarak etki alanı sınıflarından öznitelikleri ve alt düğümleri devralın:

- `Color` ve `Line``Style` öznitelikleri.

- **ExposesFillColorAsProperty** ve birkaç benzer öznitelik. Bu Boole öznitelikleri, ilgili özellik değişkenlerini kullanıcıya göre gösterir. Genellikle dil kullanıcısı diyagramda bir şekle tıkladığında Özellikler  penceresinde görünen özellikler, şeklin eşlenmiş olduğu etki alanı sınıfı örneğinin özellikleridir. true `ExposesFillColorAsProperty` olarak ayarlanırsa şeklin bir özelliği de görünür.

- **ShapeHasDecorators**. Her metin, simge veya genişlet/daralt dekoratörü için bu özniteliğin bir örneği oluşur. (DslDefinition.dsl dosyasında, `ShapeHasDecorators` true olarak ayarlanmış `UseFullForm` bir ilişkidir.)

## <a name="shape-maps"></a>Şekil Haritaları

Şekil eşlemeleri, belirli bir etki alanı sınıfının örneklerinin bir şekille temsil edilen ekranda nasıl görünerek görüntül olduğunu belirler. Hem şekil hem de bağlayıcı eşlemeleri `Diagram` DslDefinition.dsl dosyasının bölümünde görünür.

Aşağıdaki örnekte olduğu gibi, öğelerin en azından etki alanı sınıfının bilinen adı, şeklin bilinen adı ve bir `ShapeMap` öğesi `ParentElementPath` vardır:
...
öğesinin birincil `ParentElementPath` işlevi, aynı nesne sınıfının farklı bağlamlarda farklı bir şekil olarak görünmesidir. Örneğin, bir `InPort` açıklamaya da eklenmişse, bu `InPort` amaç için farklı bir şekil olarak görünebilir.

İkinci olarak, yol şeklin üst öğeyle nasıl ilişkili olduğunu belirler. DslDefinition.dsl dosyasındaki şekiller arasında ekleme yapısı tanımlanmamıştır. Yapıyı şekil eşlemelerinden çıkarabilirsiniz. Şeklin üst öğesi, üst öğe yolunun tanımları olan etki alanı öğesine eşlenen şekildir. Bu durumda yol, ait olduğu bileşeni `InPort` tanımlar. Başka bir şekil haritasında Component sınıfı ComponentShape ile eşlenmiş. Bu nedenle, `InPort` yeni şekil bileşeninin alt şeklinde `ComponentShape` yapılır.

Bunun yerine InPort şeklini diyagrama iliştirilmiş olsaydınız, üst öğe yolunun diyagramla eşlenen bileşen modeline başka bir adım atmış olması gerekir:

```
ComponentHasPorts . Component / ! Component /    ComponentModelHasComponents . ComponentModel / ! ComponentModel
```

Modelin kökünde şekil haritası yok. Bunun yerine köke doğrudan bir öğesi olan diyagramdan `Class` başvurabilirsiniz:
...
Dekoratör eşlemesi, eşlenen sınıftaki bir özelliği şekildeki bir dekoratörle ilişkilendirmektedir. özelliği numaralandı veya Boole türünde ise, değeri dekoratörün görünür olup olmadığını belirler. Dekoratör bir metin dekoratörü ise özelliğin değeri görünebilir ve kullanıcı bunu düzenleyebilir.

### <a name="compartment-shape-maps"></a>Bölme Şekli Haritaları

Bölme şekli eşlemeleri, şekil eşlemelerinin alt türleridir.

## <a name="connector-maps"></a>Bağlayıcı Eşlemeleri

En düşük bağlayıcı eşlemesi bir bağlayıcıya ve bir ilişkiye başvurur:
