...
ms.locfileid: "114703388"
---
# <a name="first-look-at-the-visual-studio-debugger"></a>Hata Ayıklayıcısı'Visual Studio bakın
...
Çoğu komut için klavye kısayolları sağlarsınız çünkü bunlar uygulama kodunda gezinmeyi daha hızlı halelar. (Menü komutları gibi eşdeğer komutlar parantez içinde gösterilir.)
...
Bazen hata ayıklama oturuma devam etmek ancak hata ayıklayıcıyı geçerli işlevde ilerleyerek devam etmek istiyor olabilirsiniz.
...
Kodu düzenlerken (hata ayıklayıcıda duraklatmak yerine), uygulamanıza bir kod satırına sağ tıklayın ve Imleçte Çalıştır'ı **seçin** (veya **Ctrl'den** **F10'a basın).** Bu komut hata ayıklamayı başlatır ve geçerli kod satırı üzerinde geçici bir kesme noktası ayarlar.
...
Tüm özelliklerini (bu örnekteki özelliği gibi) `FullPath` görmek için nesnesini genişletin.
...
Daha fazla bilgi için bkz [. izleme ve hızlı izleme kullanarak bir Izleme ayarlama Windows](../debugger/watch-and-quickwatch-windows.md)
...
## <a name="examine-an-exception"></a><a name="exception"></a> Özel durumu inceleyin
