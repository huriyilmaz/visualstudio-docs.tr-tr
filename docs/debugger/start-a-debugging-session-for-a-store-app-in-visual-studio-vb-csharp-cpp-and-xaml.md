...
description: Evrensel Visual Studio Platformu (UWP) uygulaması için bir Windows hata ayıklama oturumu başlatma. Hata ayıklama oturumunu yapılandırarak uygulamayı başlatmanın yolunu seçin.
...
ms.locfileid: "126627872"
---
# <a name="start-a-debugging-session-for-a-uwp-app"></a>UWP uygulaması için hata ayıklama oturumu başlatma

Bu makalede, Bir Evrensel Windows Platformu (UWP) uygulaması için bir Visual Studio hata ayıklama oturumu başlatma açıklanmıştır. UWP uygulamaları XAML ve C++, XAML ve C#/Visual Basic. UWP uygulamasında hata ayıklamaya başlamak için hata ayıklama oturumunu yapılandırarak uygulamayı başlatmanın yolunu seçin.
...
### <a name="choose-the-debugger-to-use"></a><a name="BKMK_Choose_the_debugger_to_use"></a> Kullanmak istediğiniz hata ayıklayıcıyı seçin
...
Evrensel **(Şifrelenmemiş Protokol)** kimlik doğrulama modu IoT, Xbox ve HoloLens cihazları ve Oluşturucu Güncelleştirmesi veya sonraki Windows 10 bilgisayarlara yöneliktir.

**Kimlik doğrulama yöntemini değiştirmek için:**

- C# ve Visual Basic için Hata ayıklama özelliği **sayfasında** Hedef cihaz **olarak Uzak** **Makine'yi seçin.** Ardından, Kimlik **Doğrulama Modu** için Yok veya **Evrensel (Şifrelenmemiş Protokol)** **öğesini seçin.**
...
Hata AyıklamaYı **Başlat Hata**  >  **Ayıklamayı Başlat'ı** veya **F5** tuşuna Visual Studio hata ayıklayıcı eklenmiş olarak uygulamayı başlatabilirsiniz. Yürütme bir kesme noktası ulaşıncaya kadar devam eder, yürütmeyi el ile askıya alırsınız, işlanmamış bir özel durum oluşur veya uygulama sona erer.
...
Varsayılan olarak, Visual Studio hata ayıklamaya başlarken uygulamayı hemen başlatır. Ayrıca uygulamayı hata ayıklama modunda çalıştıracak şekilde de ayarlayın, ancak uygulamayı hata ayıklayıcısı dışında başlatın. Örneğin, Uygulama başlat menüsünden uygulama başlatma Windows  veya uygulamadaki bir arka plan işleminin hata ayıklaması yapmak istiyor olabilir. Bu seçeneği belirtirseniz, uygulama başlatma sırasında hata ayıklayıcısında başlatılır.
...
Hata ayıklayıcıyı bir uygulamaya eklemek için, uygulamayı hata ayıklama modunda Paket Yöneticisi hata ayıklanabilir hata [!INCLUDE[win8_appname_long](../debugger/includes/win8_appname_long_md.md)] ayıklayıcısını kullansanız iyi olur. Hata ayıklanabilir Paket Yöneticisi, Visual Studio için Uzak Araçlar.
...
1. Windows **ekranında Hata** Ayıklanabilir'i için arama ve başlatma **Paket Yöneticisi.**
...
1. Kod **Türü Seç iletişim** kutusundan birini seçin:
   ...
   - **Bu kod türlerinde hata ayıkla** ve sonra listeden bir veya daha fazla kod türü seçin.
...
::: moniker range="vs-2017"
> [!NOTE]
> JavaScript uygulamaları, uygulama oluşturma işleminin *birwwahost.exe* çalışır. Birden fazla JavaScript uygulaması çalıştırıldısa, uygulamanıza eklemek için, uygulamanıza uygulamanıza  uygulamanınwwahost.exekimliğini (PID) biliyor gerekir.
>
> JavaScript uygulamanıza eklemenin en kolay yolu diğer tüm JavaScript uygulamalarını kapatmaktır. Ya da, uygulamanıza başlamadan önce *wwahost.exe* Görev Yöneticisi'nde Windows çalıştırmaya yönelik PID'leri not edin. Uygulamanıza başsanız, *wwahost.exe* PID, daha önce not edilenlerden farklı olan pin'tir.
::: moniker-end
