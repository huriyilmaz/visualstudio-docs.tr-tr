...
Visual Studio 2017 ' de, bu makalede gösterilen komutların ve seçeneklerin çoğu, HTML ve JavaScript için UWP uygulamaları için de geçerlidir. Komutları yönetilen ve C++ uygulamalar arasında farklı olduğunda, JavaScript uygulamaları genellikle UWP uygulamalarına yönelik C++ komutlarla aynıdır.
::: moniker-end

## <a name="BKMK_The_easy_way_to_start_debugging"></a>Visual Studio araç çubuğundan hata ayıklamayı Başlat
...
1. Hata ayıklamayı başlatmak için araç çubuğunda yeşil **Başlangıç** okunu veya **Hata Ayıkla** > **hata ayıklamayı Başlat**' ı seçin veya **F5**tuşuna basın.
...
Visual Studio araç çubuğunda veya projenin hata ayıklama özelliği sayfasında hata ayıklama hedefini ayarlayabilirsiniz. Aşağıdaki seçeneklerden birini belirleyin:
...
|**Aygıtların**|USB bağlantılı bir cihazda uygulamada hata ayıklayın. Cihazın, geliştirici kilidi açılmış ve ekranın kilidinin açık olması gerekir.|
...
## <a name="BKMK_Open_the_debugging_property_page_for_the_project"></a>Proje özellik sayfasında hata ayıklamayı yapılandırma
...
1. **Özellikler** bölmesinin sol tarafında:
   ...
   - Ve C# Visual Basic uygulamalar Için **Hata Ayıkla**' yı seçin.

     ![C#ve Visual Basic proje hata ayıklama özellik sayfası](../debugger/media/dbg_csvb_debugpropertypage.png)

   - Uygulamalar C++ için  > **hata ayıklama** **yapılandırma özellikleri**' ni seçin.
...
Ve C# Visual Basic uygulamalar Için, Visual Studio varsayılan olarak yönetilen kod olarak hata ayıklama ekler. Diğer veya ek kod türlerinde hata ayıklamayı seçebilirsiniz. Projenin bir parçası olan herhangi bir arka plan görevi için **hata ayıklayıcı tür** değerlerini de ayarlayabilirsiniz.

C++ Uygulamalarda, Visual Studio varsayılan olarak yerel kodu hata ayıklayıcıya ekler. Yerel koda veya buna ek olarak, belirli kod türlerinde hata ayıklamayı seçebilirsiniz.

**Hata ayıklamak için kod türlerini belirtmek için:**

- Ve C# Visual Basic uygulamalar Için, **hata ayıklama** özelliği sayfasında **hata ayıklayıcı türü** altında, **uygulama türü** ve **arka plan işlem türü** açılan listesinden birini seçin.

- Uygulamalar C++ Için **hata ayıklama** özellik sayfasındaki **hata ayıklayıcı türü** açılır listesinden aşağıdaki hata ayıklayıcılarından birini seçin.

|||
|-|-|
|**Yalnızca yönetilen**|Uygulamanızdaki yönetilen kodda hata ayıklayın. JavaScript kodu ve yerel C/C++ kod yok sayılır.|
|**Yalnızca yerel**|Uygulamanızda Yerel C/C++ kodda hata ayıklayın. Yönetilen kod ve JavaScript kodu yok sayılır.|
|**Karışık (yönetilen ve yerel)**|Uygulamanızda Yerel C/C++ kod ve yönetilen kod hatalarını ayıklayın. JavaScript kodu yoksayıldı. C++ Projelerde, bu seçenek **yönetilen ve yerel**olarak adlandırılır.|
|**Komut Dosyası**|Uygulamanızdaki JavaScript kodunda hata ayıklayın. Yönetilen kod ve yerel kod yok sayılır.|
|**Betiği ile yerel**|Uygulamanızda Yerel C/C++ Code ve JavaScript kodu hatalarını ayıklayın. Yönetilen kod yok sayılır. Yalnızca C++ projelerde veya arka plan görevlerinde kullanılabilir.|
|**Yalnızca GPU (C++ amp)**|Grafik işleme C++ BIRIMI (GPU) üzerinde çalışan yerel kodun hatalarını ayıklayın. Yalnızca C++ projelerde kullanılabilir.|

### <a name="BKMK__Optional__Disable_network_loopbacks"></a>Ağ geri alma işlemlerini devre dışı bırak (isteğe bağlı)
...
- Ve C# Visual Basic uygulamalar Için, **hata ayıklama** özelliği sayfasında **Başlangıç seçenekleri** altında **yerel ağ geri döngüsüne izin ver** onay kutusunu işaretleyin.

- Uygulamalar C++ Için **hata ayıklama** Özellik sayfasında **yerel ağa izin ver geri döngüsü** açılır listesinden **Hayır** ' ı seçin.

### <a name="BKMK__Optional__Reinstall_the_app_when_you_start_debugging"></a>Hata ayıklamayı başlattığınızda uygulamayı yeniden yükleme (isteğe bağlı)
 Veya Visual Basic bir C# uygulamayla ilgili yükleme sorunlarını tanılamak için Kaldır ' ı seçin ve ardından **hata ayıklama** özelliği sayfasında **paketmi yeniden yükleme** ' yi seçin. Bu seçenek, hata ayıklamayı başlattığınızda özgün yüklemeyi yeniden oluşturur. Bu seçenek projeler için C++ kullanılamaz.

### <a name="BKMK__Optional__Disable_authentication_requirement_to_start_the_remote_debugger"></a>Uzaktan hata ayıklama için kimlik doğrulama seçeneklerini ayarla
...
- Ve C# Visual Basic uygulamalar Için, **hata ayıklama** özelliği sayfasında, **hedef cihaz**olarak **uzak makine** ' yi seçin. Ardından, **kimlik doğrulama modu**için **hiçbiri** veya **Evrensel (şifrelenmemiş protokol)** seçeneğini belirleyin.

- Uygulamalar C++ Için hata ayıklayıcı altında **uzak makine** ' yi seçerek **hata ayıklama** Özellik sayfasında **başlatın** . Ardından **kimlik doğrulama türü**Için **kimlik doğrulaması yok** veya **Evrensel (şifrelenmemiş protokol)** seçeneğini belirleyin.
...
## <a name="BKMK_Start_the_debugging_session"></a>Hata ayıklama başlatma seçenekleri

**Hata ayıklama @no__t-** 1 ' i seçtiğinizde**hata ayıklamayı başlatın** veya **F5**tuşuna basarsanız, Visual Studio uygulamayı hata ayıklayıcı ekli olarak başlatır. Yürütme, kesme noktasına ulaşılana kadar devam eder, yürütmeyi el ile askıya alın, işlenmeyen bir özel durum oluşur veya uygulama sonlanır.

### <a name="BKMK_Start_debugging__F5__but_delay_the_app_start"></a>Hata ayıklamayı Başlat, ancak uygulama başlangıcını geciktir
...
- Ve C# Visual Basic uygulamalar Için, **hata ayıklama** özelliği sayfasında başlatma **seçenekleri** altında **Başlarken ' i seçin ancak hata ayıkla '** yı seçin.

- Uygulamalar C++ Için **hata ayıklama** Özellik sayfasında **Uygulamayı Başlat** açılan listesinden **Hayır** ' ı seçin.
...
### <a name="BKMK_Start_an_installed_app_in_the_debugger"></a>Yüklü veya çalışan bir UWP uygulamasında hata ayıklama
...
Hata ayıklayıcıda yüklü veya çalışan bir UWP uygulamasını başlatmak için **hata**Ayıkla  > **diğer hata ayıklama hedefleri** > **hata ayıklama yüklü uygulama paketi**' ni seçin. Daha fazla yönerge için bkz. [yüklü uygulama paketinin hatalarını ayıklama](../debugger/debug-installed-app-package.md).

### <a name="BKMK_Attach_the_debugger_to_a_running_app_"></a>Hata ayıklayıcıyı çalışan bir Windows 8. x uygulamasına iliştirme

Hata ayıklayıcıyı [!INCLUDE[win8_appname_long](../debugger/includes/win8_appname_long_md.md)] uygulamasına eklemek için, uygulamayı hata ayıklama modunda çalışacak şekilde ayarlamak için hata ayıklanabilir Paket Yöneticisi ' ni kullanmanız gerekir. Hata ayıklanabilir Paket Yöneticisi Visual Studio için Uzak Araçlar ile yüklenir.
...
1. Uygulamanın PackageFullName tanıtıcısını belirtin.
   ...
   1. Tüm uygulamaların PackageFullName 'sini içeren bir listeyi görüntülemek için PowerShell komut isteminde `Get-AppxPackage` yazın.

   1. PowerShell komut isteminde `Enable-AppxDebug <PackageFullName>` girin; burada \<PackageFullName > uygulamanın PackageFullName tanımlayıcısıdır.

1. **Hata ayıkla**@no__t-**İşleme İliştir**' i seçin.
...
::: moniker range="vs-2017"
> [!NOTE]
> JavaScript uygulamaları, *wwahost. exe* işleminin bir örneğinde çalışır. Birden çok JavaScript uygulaması çalışıyorsa, buna iliştirilecek uygulamanızın *wwahost. exe* işleminin sayısal işlem KIMLIĞINI (PID) bilmeniz gerekecektir.
>
> JavaScript uygulamanıza eklemenin en kolay yolu, diğer tüm JavaScript uygulamalarını kapatmanız. Ya da, uygulamanızı başlamadan önce Windows Görev Yöneticisi 'nde *wwahost. exe* Işlemlerinin çalıştırıldığı PID 'leri görebilirsiniz. Uygulamanızı başlattığınızda, *wwahost. exe* PID dosyası, daha önce belirtilenlerden farklı olur.
::: moniker-end
