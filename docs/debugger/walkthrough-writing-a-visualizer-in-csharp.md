...
* Yeni bir sınıf kitaplığı projesi oluşturun.
    ...
    Dosya **Yeni**  >  **Dosya'Project.**  >   Dil açılan listesinde **C# seçin.** Arama kutusuna sınıf kitaplığı yazın **ve ardından** Sınıf Kitaplığı **(sınıf kitaplığı) .NET Framework seçin.** **İleri**’ye tıklayın. Görüntülenen iletişim kutusuna adını yazın ve `MyFirstVisualizer` Oluştur'a **tıklayın.**

    Görselleştirici projesi için .NET değil, .NET Framework sınıf kitaplığını seçin. Görselleştiricinin farklı olması .NET Framework çağıran uygulama .NET olabilir.
    ::: moniker-end
...
- içinde `public class DebuggerSide` aşağıdaki yöntemi **ekleyin:**
  ...
  yöntemi, görselleştirici iletişim kutusunu veya diğer kullanıcı arabirimini oluşturan kodu içerir ve hata ayıklayıcıdan `Show` görselleştiriciye geçirilen bilgileri görüntüler. İletişim kutusunu oluşturan ve bilgileri görüntüleyen kodu eklemeniz gerekir. Bu kılavuzda, bunu Formlar ileti Windows kullanarak yapacaksınız. İlk `using` olarak, System.Windows için bir başvuru ve yönerge eklemeniz gerekir. Forms.
...
4. DebuggerSide.cs dosyasında, yönergelerine `using` aşağıdakini ekleyin:
   ...
   Şimdi görselleştiricinizin kullanıcı arabirimini oluşturmak ve göstermek için bazı kodlar eksersiniz. Bu ilk görselleştiriciniz olduğundan, kullanıcı arabirimini basit tutacak ve bir Message Box kullanmayacak.
...
2. Build **(Derleme)** menüsünde Build **MyFirstVisualizer (Derleme MyFirstVisualizer) öğesini seçin.** Projenin başarıyla derlemesi gerekir. Devam etmeden önce derleme hatalarını düzeltin.

   Bu, hata ayıklayıcı yan kodunun sonudur. Ancak bir adım daha vardır; hata ayıklayıcı tarafına hangi sınıf koleksiyonunun görselleştiriciyi olduğunu söyleyen özniteliği.
...
1. Bu Çözüm Gezgini, çözüme sağ tıklayın, Ekle'yi **seçin** ve ardından Yeni **Project.**
    ...
    Dosya **Yeni**  >  **Dosya'Project.**  >   Dil açılan listesinde **C# seçin.** Arama kutusuna konsol uygulaması **yazın ve ardından**.NET için Konsol Uygulaması **(.NET Framework)** veya **Konsol Uygulaması'nu** seçin. **İleri**’ye tıklayın. Görüntülenen iletişim kutusuna adını yazın ve `MyTestConsole` Oluştur'a **tıklayın.**

    > [!NOTE]
    > Görselleştiriciyi test donanımı kullanarak kolayca test etmek için bir .NET Framework oluşturun. Bunun yerine bir .NET konsol uygulaması oluşturabilirsiniz, ancak daha sonra açıklanan test yararlanıcısı henüz .NET için desteklenmiyor, bu nedenle test etmek için görselleştiriciyi yüklemeniz gerekir. Bir .NET konsol uygulaması için, önce burada konsol uygulamasını oluşturun, gerekli DLL ve proje başvurularını ekleyin ve ardından Hata ayıklama tarafı veri nesnesi ekleme konusunda [açıklanan adımları izleyin.](#add-a-debuggee-side-data-object) Daha ASP.NET Core için bkz. [.NET 5.0+ için](../debugger/create-custom-visualizers-of-data.md#special-debugger-side-considerations-for-net-50)özel hata ayıklayıcı tarafı konuları.
    ::: moniker-end
    ::: moniker range="vs-2017"
