...
### <a name="Workflow"></a>UI yanıtlama hızı sorununu yalıtma
...
2. Uygulamanızı UI yanıtlama hızı sorunları için test edin. (Hata ayıklama olmadan uygulamanızı başlatmak için **Ctrl**+**F5** tuşuna basın.)
...
3. Visual Studio 'ya geçin ( **Alt**+**sekmesine**basın) ve uygulamanızı durdurun (**SHIFT**+**F5**).
...
|Boyayın|Çizmeye|DOM 'da görsel değişiklikler yapıldı ve sayfanın bölümlerini yeniden oluşturma girişiminde bulunuldu.|
|İşleme katmanı|Çizmeye|DOM 'ın bağımsız olarak işlenmiş bir parçasında (katman olarak adlandırılır) görsel değişiklikler yapılmıştır ve değişiklikler, sayfanın işlenmesi için gereken bir kısmı gerektirdi.|
