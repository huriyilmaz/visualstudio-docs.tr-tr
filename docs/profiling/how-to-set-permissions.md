...
5. **\<Kullanıcı adı >** kutusunda, **tam denetim**onay kutusunu seçin.
...
4. Bilgisayara yönetici erişimi olmayan bir kullanıcı veya grup tarafından profil oluşturma sürücüsü işlevine erişime izin vermek için komutunu çalıştırın:
    ...
    ```
     ...
     Bu komut \<Kullanıcı adı > veya \<grup adı > Hesap profil oluşturma araçlarına erişim verir. \<right > seçeneği, kullanıcının erişebileceği profil oluşturma işlevini belirler. \<right > seçeneği aşağıdaki değerlerden biri veya daha fazlası olabilir:
