...
Bir yöntem `Get`ile başlayan bir ada sahiptir, hiçbir parametre kullanmaz ve dizi olmayan bir değer döndürür.
...
<xref:System.Diagnostics.DebuggerBrowsableAttribute?displayProperty=fullName>uygulayarak hata ayıklayıcının özellikleri genişletmeyi engelleyebilirsiniz. Aşağıdaki örnek, bir örnek özelliğine uygulanan bu özniteliği gösterir.
