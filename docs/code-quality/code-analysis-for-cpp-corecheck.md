...
ms.locfileid: "72745961"
---
# <a name="c-core-guidelines-checker-reference"></a>C++Temel kılavuz denetleyicisi başvurusu

Bu bölümde, C++ ana yönergeler denetçisi uyarıları listelenmektedir. Kod Analizi hakkında daha fazla bilgi için bkz. [/analyze (kod analizi)](/cpp/build/reference/analyze-code-analysis) ve [hızlı başlangıç: C/C++için kod analizi](../code-quality/quick-start-code-analysis-for-c-cpp.md).
...
[C26431 DONT_TEST_NOTNULL](C26431.md) '% Expr% ' ifadesinin türü zaten GSL:: not_null. Bu dosyayı nulllik için test etme. Bkz [ C++ . temel yönergeler F. 23](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md#f23-use-a-not_nullt-to-indicate-that-null-is-not-a-valid-value).
...
[C26481 NO_POINTER_ARITHMETIC](C26481.md) İşaretçi aritmetiği kullanmayın. Bunun yerine span kullanın. Bkz [ C++ . temel kılavuz sınırları. 1](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md#SS-bounds).
...
[C26415 SMART_PTR_NOT_NEEDED](C26415.md) '% Symbol% ' akıllı işaretçi parametresi yalnızca içerilen işaretçiye erişmek için kullanılır. Bunun yerine T * veya T & kullanın. Bkz [ C++ . temel yönergeler R. 30](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md#Rr-smartptrparam).
...
[C26417 NO_LVALUE_REF_SHARED_PTR](C26417.md) '% Symbol% ' paylaşılan işaretçi parametresi, başvuruya göre geçirildi ve sıfırlanmadı veya yeniden atandı. Bunun yerine T * veya T & kullanın. Bkz [ C++ . temel yönergeler R. 35](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md#Rr-sharedptrparam).

[C26418 NO_VALUE_OR_CONST_REF_SHARED_PTR](C26418.md) '% Symbol% ' paylaşılan işaretçi parametresi kopyalanmadı veya taşınmadı. Bunun yerine T * veya T & kullanın. Bkz [ C++ . temel yönergeler R. 36](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md#Rr-sharedptrparam-const).
...
[C26461 USE_CONST_POINTER_ARGUMENTS](c26461.md): '% function% ' işlevi için '% Argument% ' işaretçi bağımsız değişkeni `const`için bir işaretçi olarak işaretlenebilir. Bkz [ C++ . temel yönergeler Con. 3](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md#Rconst-ref).
...
[C26466 NO_STATIC_DOWNCAST_POLYMORPHIC](c26466.md) `static_cast` Alta atamaları kullanmayın. Polimorfik bir türden atama dynamic_cast kullanmalıdır. Bkz [ C++ . temel yönergeler türü. 2](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md#Pro-type-downcast).
...
[C26494 VAR_USE_BEFORE_INIT](c26494.md) '% Variable% ' değişkeni başlatılmamış. Her zaman bir nesne başlatın. Bkz [ C++ . temel yönergeler türü. 5](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md#SS-type).
...
İşaretçi aritmetiği kullanmayın. Bunun yerine span kullanın. Bkz [ C++ . temel kılavuz sınırları. 1](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md#SS-bounds)
...
[C26483 STATIC_INDEX_OUT_OF_RANGE](c26483.md) % Value% değeri sınırların dışında (%0,% sınır%) '% Variable% ' değişkeninin. Yalnızca dizi sınırları içindeki sabit ifadeleri kullanarak dizilere dizin ekleyin. Bkz [ C++ . temel kılavuz sınırları. 2](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md#SS-bounds)
...
Geçici [C26449 NO_SPAN_FROM_TEMPORARY](c26449.md)
`gsl::span` veya `std::string_view` geçici bir şekilde geçersiz kılındığınızda geçersiz olur. Bkz [ C++ . temel yönergeler: GSL. View: views](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md#gslview-views).

## <a name="deprecated-warnings"></a>Kullanım dışı uyarıları

Aşağıdaki uyarılar, temel kılavuz denetleyicinin erken deneysel kural kümesinde bulunur, ancak artık kullanımdan kaldırılmıştır ve güvenle yoksayılabilir. Uyarılar, yukarıdaki listede yer alan uyarılarla yerini almıştır.
