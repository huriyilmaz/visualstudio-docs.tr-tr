...
- @No__t_0 yöntemi, güvenli olmayan bir <xref:System.Xml.XmlReaderSettings> örneğiyle veya hiç örnek olmadan çağrılır.
...
- @No__t_0 özelliğinin güvenilen bir kaynaktan atandığından emin olun.
...
- @No__t_0 özelliğini **yasakla** veya **Yoksay**olarak ayarlayarak güvenilmeyen kaynaklarla uğraşıyorsanız DtdProcessing 'ı etkinleştirmemeye özen gösterin.
...
> [!NOTE]
> @No__t_0, <xref:System.Xml.XmlReader> bir örnek oluşturmak için önerilen yöntem olsa da <xref:System.Xml.XmlTextReader> davranış farklılıkları vardır. Bir <xref:System.Xml.XmlReader>, XML değerlerinde `\n` `\r\n` <xref:System.Xml.XmlReader.Create%2A> normalleştirir <xref:System.Xml.XmlTextReader> sırasını korur.
