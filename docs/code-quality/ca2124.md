...
.NET Framework 1,0 sürümleri ve 1,1 ' de, genel veya korumalı bir yöntem bir `try`/`catch`/`finally` bloğunu içerir. `finally` bloğu güvenlik durumunu sıfırlayıp bir `finally` bloğunun içine alınmaz.
...
Sarmalanmamış `try`/`finally` bir dış try bloğuna yerleştirin. Aşağıdaki ikinci örneğe bakın. Bu, `finally` kodu filtre yapmadan önce yürütmeye zorlar.
