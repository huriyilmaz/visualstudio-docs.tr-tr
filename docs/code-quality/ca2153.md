...
CSE, bir işlemin durumunun bozuk olduğunu ve sistem tarafından yakalanmadığını gösterir. Bozuk durum senaryosunda, bir genel işleyici yalnızca <xref:System.Runtime.ExceptionServices.HandleProcessCorruptedStateExceptionsAttribute?displayProperty=fullName> özniteliği ile yönteminizin işaretlenmesi durumunda özel durumu yakalar. Varsayılan olarak, [ortak dil çalışma zamanı (CLR)](/dotnet/standard/clr) CSEs için catch işleyicilerini çağırmaz.
...
Bu uyarı, tüm özel durumları yakalayan genel bir işleyiciyle (örneğin, `catch (System.Exception e)` veya özel durum parametresi olmayan `catch`), CSEs yakalamada tetiklenir.

## <a name="how-to-fix-violations"></a>İhlalleri çözme

Bu uyarıyı çözmek için aşağıdakilerden birini yapın:

- <xref:System.Runtime.ExceptionServices.HandleProcessCorruptedStateExceptionsAttribute> özniteliğini kaldırın. Bu, CSEs 'nin catch işleyicilerine geçirilmediği varsayılan çalışma zamanı davranışına geri döner.
...
<xref:System.Runtime.ExceptionServices.HandleProcessCorruptedStateExceptionsAttribute> özniteliğini kaldırmak, bozuk durum özel durumlarının yönteminizin tarafından işlenmemesini sağlar.
