...
<xref:System.AttributeTargets?displayProperty=fullName> numaralandırması, özel bir öznitelik için belirtebileceğiniz hedefleri tanımlar. <xref:System.AttributeUsageAttribute>atlarsanız, özel öznitele, <xref:System.AttributeTargets> numaralandırmanın `All` değeri tarafından tanımlanan tüm hedefler için geçerli olacaktır.
...
Aşağıdaki örnek iki özniteliği tanımlar. `BadCodeMaintainerAttribute`, <xref:System.AttributeUsageAttribute> deyiminden yanlış atlar ve bu bölümde daha önce açıklanan özniteliği doğru bir şekilde uygular `GoodCodeMaintainerAttribute`. Özellik `DeveloperName` tasarım kuralı [için gereklidir: öznitelik bağımsız değişkenleri için erişimcileri tanımlayın](../code-quality/ca1019.md) ve tamamlana için eklenmiştir.
