...
@No__t_0 türünün yerel bir nesnesi oluşturulur, ancak nesnenin tüm başvuruları kapsam dışına alınmadan önce nesne atılamaz.
...
@No__t_4 uygulayan nesneleri kaydırmak için [`using` ifadesini](/dotnet/csharp/language-reference/keywords/using-statement) ([ `Using`](/dotnet/visual-basic/language-reference/statements/using-statement) Visual Basic) kullanabilirsiniz. Bu şekilde Sarmalanan nesneler `using` bloğunun sonuna otomatik olarak silinir. Ancak, aşağıdaki durumlar `using` ifadesiyle işlenmemelidir:
