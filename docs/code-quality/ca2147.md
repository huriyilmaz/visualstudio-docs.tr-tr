...
@No__t-0 olarak işaretlenen koda, onaylama için yeterli izin verilmez.

## <a name="rule-description"></a>Kural açıklaması
Bu kural,% 100 saydam ya da saydam/kritik olan bir derlemedeki tüm yöntemleri ve türleri analiz eder ve <xref:System.Security.CodeAccessPermission.Assert%2A> ' ın herhangi bir bildirim veya kesinlik kullanımını işaretler.

Çalışma zamanında, saydam koddan <xref:System.Security.CodeAccessPermission.Assert%2A> ' a yapılan çağrılar <xref:System.InvalidOperationException> oluşturulmasına neden olur. Bu, hem% 100 saydam derlemede hem de bir yöntem ya da türün saydam olarak bildirildiği, ancak bildirime dayalı veya zorunlu bir onay içeren karışık saydam/kritik derlemelerde meydana gelebilir.
...
@No__t-0 saydamsa, `Assert` yöntemi bir <xref:System.InvalidOperationException> oluşturduğunda bu kod başarısız olur.
