...
.NET Framework 2,0 ' dan önce, olay işleyicisine özel bilgi geçirmek için, <xref:System.EventArgs?displayProperty=fullName> sınıfından türetilmiş bir sınıfı belirtilen yeni bir temsilcinin bildirilmesini gerekiyordu. .NET Framework 2,0 ve sonraki sürümlerinde, genel <xref:System.EventHandler%601?displayProperty=fullName> temsilcisi, <xref:System.EventArgs> ' den türetilen tüm sınıfın olay işleyicisiyle birlikte kullanılmasına izin verir.
...
Aşağıdaki kod parçacığı, kuralını karşılayan önceki örnekteki temsilci bildirimini kaldırır. `ClassThatRaisesEvent` ve `ClassThatHandlesEvent` yöntemlerinde kullanımını <xref:System.EventHandler%601?displayProperty=fullName> temsilciyi kullanarak değiştirir.
