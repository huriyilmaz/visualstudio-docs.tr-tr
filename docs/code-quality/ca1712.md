...
|`dotnet_code_quality.CA1712.enum_values_prefix_trigger = Heuristic` | Kural, varsayılan FxCop buluşsal yöntemi kullanılarak tetiklenir (yani, numaralandırma değerlerinin en az %75 ' ı numaralandırma türü adıyla başlamadığında).

Bu seçeneklerin tümünü yalnızca bu kural için, tüm kurallar için veya bu kategorideki tüm kurallar (adlandırma) için yapılandırabilirsiniz. Daha fazla bilgi için bkz. [FxCop çözümleyicileri yapılandırma](configure-fxcop-analyzers.md).
