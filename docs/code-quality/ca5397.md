...
- Kullanım dışı <xref:System.Security.Authentication.SslProtocols?displayProperty=nameWithType> bir değere başvuruldu.
- Kullanım dışı bırakılan bir değeri temsil eden bir tamsayı değeri, <xref:System.Security.Authentication.SslProtocols> dönüş değeri olarak kullanılan veya <xref:System.Security.Authentication.SslProtocols> bağımsız değişkeni olarak kullanılan bir <xref:System.Security.Authentication.SslProtocols> değişkenine atandı.
...
[CA5398: sabit kodlanmış SslProtocols değerlerinden kaçının](ca5398.md)
