...
@No__t-0 gibi karma işlevler ve <xref:System.Security.Cryptography.DES> ile <xref:System.Security.Cryptography.RC2> gibi şifreleme algoritmaları önemli risk gösterebilir ve bu durum, deneme yanılma saldırıları ve karma çarpışmaları gibi önemsiz saldırı teknikleriyle hassas bilgilerin açığa çıkmasına neden olabilir.

Aşağıdaki şifreleme algoritmaları listesi, bilinen şifreleme saldırılarına tabidir. @No__t-0 şifreleme karma algoritması, karma çakışma saldırılarına tabidir.  Bir karma çarpışması, kullanıma bağlı olarak, karma işlevinin benzersiz şifreleme çıktısına bağlı olan sistemlerde kimliğe bürünme, izinsiz değişiklik veya diğer saldırı türlerine yol açabilir. @No__t-0 ve <xref:System.Security.Cryptography.RC2> şifreleme algoritmaları şifrelenmiş verilerin istenmeden açıklanmasına neden olabilecek şifreleme saldırılarına tabidir.
