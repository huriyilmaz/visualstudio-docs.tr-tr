...
- Tür devralınır, ancak <xref:System.Runtime.Serialization.ISerializable.GetObjectData%2A?displayProperty=fullName> yöntemini geçersiz kılmaz ve tür <xref:System.NonSerializedAttribute?displayProperty=fullName> özniteliğiyle işaretlenmemiş örnek alanlarını bildiriyor.
...
## <a name="example"></a>Örnek
Aşağıdaki örnek, kitap sınıfında <xref:System.Runtime.Serialization.ISerializable.GetObjectData> ' a ait bir @no__t geçersiz bir uygulama sağlayarak ve kitaplık sınıfında bir-1 uygulamasını sağlayarak önceki iki ihlali düzeltir.
