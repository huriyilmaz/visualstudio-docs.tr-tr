...
Adlandırma kuralları, bir numaralandırma için çoğul adının aynı anda birden fazla sabit değer olarak belirtilebileceğini gösterir. <xref:System.FlagsAttribute>, derleyicilerin numaralandırma üzerinde bit düzeyinde işlemler sağlayan bir bit alanı olarak değerlendirilmesi gerektiğini belirtir.

Tek seferde bir sabit listesinin yalnızca bir değeri belirtilenebilir, numaralandırmanın adı tekil bir sözcük olmalıdır. Örneğin, haftanın günlerini tanımlayan bir numaralandırma, birden çok gün belirtebileceğiniz bir uygulamada kullanılmak üzere tasarlanmış olabilir. Bu numaralandırma <xref:System.FlagsAttribute> sahip olmalıdır ve ' Days ' olarak çağrılabilir. Yalnızca tek bir günün belirtilmesini sağlayan benzer bir numaralandırma özniteliğe sahip değildir ve ' Day ' olarak çağrılabilir.
...
Sabit listesinin adını tekil bir sözcük yapın veya <xref:System.FlagsAttribute>ekleyin.
