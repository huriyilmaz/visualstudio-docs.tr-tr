...
<xref:System.Runtime.CompilerServices.RuntimeCompatibilityAttribute> veya `RuntimeCompatibility(WrapNonExceptionThrows = false)` işaretlenmiş bir derlemede, <xref:System.Exception?displayProperty=fullName> işleyen ve hemen aşağıdaki genel catch bloğunu içermeyen bir catch bloğu içeren bir üye. Bu kural [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] derlemelerini yoksayar.
...
Amaç tüm özel durumları yakalamak, genel bir catch bloğu yerine koymak veya eklemek ya da derlemeyi `RuntimeCompatibility(WrapNonExceptionThrows = true)`işaretlemek için bu kural ihlalini onarmak için. İzinler catch bloğunda kaldırılırsa, genel catch bloğundaki işlevselliği çoğaltın. Tüm özel durumları işleme amacı yoksa, <xref:System.Exception> işleyen catch bloğunu, belirli özel durum türlerini işleyen catch bloklarıyla değiştirin.
