...
Aşağıdaki örnekte, `DaysEnumNeedsFlags`, <xref:System.FlagsAttribute> ' i kullanma gereksinimlerini karşılayan ancak sahip olmadığı bir numaralandırmadır. @No__t_0 numaralandırması, iki üsleri olan ancak yanlış bir <xref:System.FlagsAttribute> belirten değer içermiyor. Bu kural CA2217 ihlal ediyor [: Numaralandırmaları FlagsAttribute ile işaretlemeyin](../code-quality/ca2217.md).
