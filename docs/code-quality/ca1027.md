...
Aşağıdaki örnekte `DaysEnumNeedsFlags`, <xref:System.FlagsAttribute> kullanma gereksinimlerini karşılayan ancak sahip olmayan bir numaralandırmadır. `ColorEnumShouldNotHaveFlag` numaralandırması, iki üsleri olan ancak yanlış bir <xref:System.FlagsAttribute>belirten değer içermiyor. Bu kural CA2217 ihlal ediyor [: Numaralandırmaları FlagsAttribute ile işaretlemeyin](../code-quality/ca2217.md).
