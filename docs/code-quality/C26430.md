...
**C++ Temel yönergeler**: F. 23: "Null" değerinin geçerli bir değer olmadığını belirtmek için bir not_null @ no__t-0T > kullanın

Kod herhangi bir zaman işaretçi değişkenlerini denetim altına alıyorsa, bunu tutarlı bir şekilde yapması ve tüm yollarda işaretçileri doğrulaması gerekir. Bazen daha fazla boş denetim, bazı durumlarda karmaşık dallardan birinde kalıcı çökme olasılığından daha iyidir. Bu tür kodların daha az karmaşık olması için yeniden düzenlenmiş olması gerekir (birden çok işleve bölünerek) ve gsl:: not_null (bkz. yönergeler Destek Kitaplığı) gibi işaretçilerin parçalarını yalıtmak için, geçerli işaretçi değerleri hakkında güvenli bir varsayım sağlayan algoritmaların parçalarını yalıtmak için. TEST_ON_ALL_PATHS kuralı, boş değer denetimlerinin tutarsız olduğu yerleri bulmaya yardımcı olur (Bu nedenle varsayımlar gözden geçirme gerektirebilir) veya olası null değerin bazı kod yollarında nulllık denetimini atlayabileceği gerçek hatalar.
