...
Boş bir <xref:System.Array>, hiçbir öğe ayrılmadı.
...
İhlalin giderilmesi için sıfır uzunluklu dizi ayırmayı <xref:System.Array.Empty%2A?displayProperty=nameWithType> ' a bir çağrı ile değiştirin. Örneğin, aşağıdaki iki kod parçacığı kural ihlalini ve nasıl düzeltileceğini gösterir:
...
> [!TIP]
> Visual Studio 'da bu kural için bir kod onarımı vardır. Kullanmak için imleci dizi ayırmaya konumlandırın ve **Ctrl**+ ' e basın **.** (nokta). Sunulan seçenekler listesinden **Array. Empty kullanın** öğesini seçin.
>
...
- [CA1814: Çok boyutlu @ no__t-0 üzerinde pürüzlü Diziler tercih etme
