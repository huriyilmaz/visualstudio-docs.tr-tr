...
| CA1060 | [CA1060: P/Invokes öğesini NativeMethods sınıfına taşıyın](../code-quality/ca1060-move-p-invokes-to-nativemethods-class.md) | System. Runtime. InteropServices. DllImportAttribute özniteliği kullanılarak işaretlenenler veya [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] ' da Declare anahtar sözcüğü kullanılarak tanımlanan Yöntemler gibi platform çağırma yöntemleri, yönetilmeyen koda erişim. Bu yöntemler, NativeMethods, SafeNativeMethods veya UnsafeNativeMethods sınıfının üyesi olmalıdır. |
...
| CA1064 | [CA1064: Özel durumlar genel olmamalıdır](../code-quality/ca1064-exceptions-should-be-public.md) | Bir iç özel durum yalnızca kendi iç kapsamı içinde görülebilir. İç kapsam dışında kalan özel durumlardan sonra, sadece basit istisnalar istisna yakalamak için kullanılabilir. İç özel durum <xref:System.Exception>, <xref:System.SystemException> veya <xref:System.ApplicationException> ' den devralınmışsa, dış kodun özel durumla ne yapacaklarından haberdar olmak için yeterli bilgileri olmayacaktır. |
...
| CA1303 | [CA1303: Harfleri yerelleştirilmiş parametreler olarak göndermeyin](../code-quality/ca1303-do-not-pass-literals-as-localized-parameters.md) | Dışarıdan görülebilen bir yöntem, bir dize sabit değerini bir .NET oluşturucusuna veya yöntemine parametre olarak geçirir ve bu dize yerelleştirilebilir olmalıdır. |
...
| CA1401 | [CA1401: P/Invoke'lar görünür olmamalıdır](../code-quality/ca1401-p-invokes-should-not-be-visible.md) | Ortak bir türdeki ortak veya korumalı bir yöntem System. Runtime. InteropServices. DllImportAttribute özniteliğine sahiptir (Ayrıca [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] ' daki Declare anahtar sözcüğü tarafından uygulanır). Bu tür yöntemler açıkta kalmamalıdır. |
...
| CA1404 | [CA1404: P/Invoke ardından hemen GetLastError çağırın](../code-quality/ca1404-call-getlasterror-immediately-after-p-invoke.md) | Marshal. GetLastWin32Error yöntemine veya eşdeğer [!INCLUDE[TLA2#tla_win32](../code-quality/includes/tla2sharptla_win32_md.md)] GetLastError işlevine bir çağrı yapılır ve hemen önceki çağrı bir işletim sistemi çağırma yöntemine değildir. |
...
| CA1415 | [CA1415: P/Invoke'ları doğru bildirin](../code-quality/ca1415-declare-p-invokes-correctly.md) | Bu kural, ÇAKıŞAN bir yapı parametresine işaretçi olan [!INCLUDE[TLA2#tla_win32](../code-quality/includes/tla2sharptla_win32_md.md)] işlevlerini hedefleyen ve karşılık gelen yönetilen parametrenin bir System. Threading. Nativesır yapısına yönelik bir işaretçi olmadığı işletim sistemi çağırma yöntemi bildirimlerini arar. |
...
| CA1822 |[CA1822: Üyeleri statik olarak işaretleyin](../code-quality/ca1822.md) | Örnek verilerine veya çağrı örnek yöntemlerine erişmeyen Üyeler statik olarak işaretlenebilir ([!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] ' da paylaşılır). Yöntemleri statik olarak işaretledikten sonra, derleyici sanal olmayan arama sitelerini bu üyelere yayar. Bu, ölçülebilir kazanç performansını performans duyarlı kodunuz için verebilir. |
...
| CA2105 | [CA2105: Dizi alanları salt okunur olmamalıdır](../code-quality/ca2105.md) |Bir dizi içeren alana salt okunur (salt okunur @no__t 0) değiştiricisini uyguladığınızda, alan farklı bir diziye başvuracak şekilde değiştirilemez. Bir dizinin öğeleri salt okunur bir alanda depolanmış olsa bile değiştirilebilir. |
