...
İç içe geçmiş `using` deyimleri (Visual Basic`Using`) CA2202 uyarı ihlallerine neden olabilir. İç içe geçmiş iç `using` deyimin IDisposable kaynağı dıştaki `using` ifadesinin kaynağını içeriyorsa, iç içe geçmiş kaynağın `Dispose` yöntemi içerilen kaynağı serbest bırakır. Bu durum oluştuğunda, dış `using` deyimin `Dispose` yöntemi kaynağı ikinci bir kez atmayı dener.

Aşağıdaki örnekte, bir dıştaki using ifadesinde oluşturulan <xref:System.IO.Stream> nesnesi, `stream` nesnesini içeren <xref:System.IO.StreamWriter> nesnesinin Dispose yönteminde Inner using ifadesinin sonunda serbest bırakılır. Dış `using` ifadesinin sonunda, `stream` nesnesi ikinci bir kez serbest bırakılır. İkinci sürüm, CA2202 ihlaline neden olur.
...
Bu sorunu çözmek için, dış `using` ifadesinin yerine bir `try`/`finally` bloğunu kullanın. `finally` bloğunda, `stream` kaynağının null olmadığından emin olun.
