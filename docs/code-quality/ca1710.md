...
<xref:System.Collections.IDictionary> uygulayan türler, tür <xref:System.Collections.IEnumerable> veya <xref:System.Collections.ICollection>de uygulayan ' sözlük ' kelimesiyle biten adlara sahiptir. ' Collection ' ve ' Dictionary ' sonek adlandırma kuralları, kullanıcıların aşağıdaki iki numaralandırma deseni arasında ayrım kurmasını sağlar.
