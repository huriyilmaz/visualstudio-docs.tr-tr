...
Aşağıdaki örnek, C# `is` işlecini kullanarak kuralı ihlal eden bir yöntemi gösterir. İkinci bir yöntem `is` işlecini bir test ile değiştirerek, iki ile yineleme başına dönüştürme işlemi sayısını azaltan `as` işlecinin sonucuna karşı bir test ile değiştirerek kuralı karşılar. Üçüncü bir yöntem ayrıca tür dönüştürme başarılı olursa istenen türde bir değişken [oluşturmak için `is`](/dotnet/csharp/language-reference/keywords/is#pattern-matching-with-is) kullanarak kuralı karşılar.
