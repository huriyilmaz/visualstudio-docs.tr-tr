...
Bir tür <xref:System.Exception?displayProperty=fullName> uzatır ancak gerekli oluşturucuların tümünü bildirmiyor.
...
İhlalin genel oluşturucular için farklı bir erişim düzeyi kullanmasından kaynaklanan bu kuraldan bir uyarıyı bastırmak güvenlidir. Ayrıca, taşınabilir bir sınıf kitaplığı (PCL) oluşturuyorsanız `NewException(SerializationInfo, StreamingContext)` Oluşturucu için uyarı bastırmaya de devam edebilirsiniz.
