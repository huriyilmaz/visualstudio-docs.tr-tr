...
Kural CA1052, türü türetilmiş bir türde geçersiz kılınabilen herhangi bir işlev sağlamadığından, yalnızca statik üyeler içeren bir türün devralınabilmesi için tasarlanmadığını varsayar. Devralınmayan bir tür, temel tür olarak kullanımını önlemek için içinde C# `static` değiştiricisi ile işaretlenmelidir. Ayrıca, varsayılan oluşturucusunun kaldırılması gerekir. Visual Basic, sınıfın bir [modüle](/dotnet/visual-basic/language-reference/statements/module-statement)dönüştürülmesi gerekir.
...
Yalnızca tür devralınacak şekilde tasarlandıysa, bu kuraldan bir uyarı gizleyin. `static` değiştiricinin yokluğu, türün temel tür olarak yararlı olduğunu önerir.
