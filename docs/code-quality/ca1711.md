...
|Bayrak | Bayraklar|<xref:System.Enum?displayProperty=fullName>|
...
Bu kuralı [FxCop çözümleyicilerinin](install-fxcop-analyzers.md) (eski analizler olmadan) çalıştırıyorsanız, her bir sonek ("|") ile ayrılmış olarak izin verilen sonekler listesini yapılandırabilirsiniz. Örneğin, kuralın bayrak ve bayraklar soneklerine karşı çalıştırılmamalıdır ve aşağıdaki anahtar-değer çiftini projenizdeki bir. editorconfig dosyasına ekleyin:
