...
geçici bir geçersiz kılındığınızda `gsl::span` veya `std::string_view`, geçici 'den oluşturulan geçersizdir.

C++Temel yönergeler: [GSL. View: Görünümler @ no__t-0.

Yayılmalar ve görünümler, bellek arabelleklerine başvurmasına izin veren kullanışlı ve hafif türlerdir. Ancak dikkatle kullanılması gerekir: arabirimi standart kapsayıcılara benzer olsa da, davranışları işaretçiler ve başvuruların davranışı gibidir. Bunlar veri sahibi değildir ve geçici arabelleklerden hiçbir şekilde oluşturulmamalıdır. Bu denetim, kaynak verilerin geçici olduğu, span veya View olmadığı durumlara odaklanır. Yayılma başvurularıyla ilgili biraz farklı senaryoyu işleyen başka bir denetim var: [C26445 NO_SPAN_REF](c26445.md). Her iki kural de, eski kod modernleştirilmiştir ve yayılmaları ya da görünümleri benimseiyorsa, hafif ancak yapılan tehlikeli hataların oluşmasını önlemeye yardımcı olabilir.
...
- Bu kural, oluşturucuların yayılmalar veya görünümler için çağrıldığı ve kaynak veri arabelleğinin aynı bildirimde oluşturulan geçici bir nesneye ait olduğu yerlerde uyarır. Buna aşağıdakiler dahildir:
  ...
  - `static_cast` ifadelerinde açık dönüşümler;
