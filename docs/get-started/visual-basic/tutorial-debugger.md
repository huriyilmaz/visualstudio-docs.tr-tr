...
Tanıtım uygulaması Visual Basic olsa da, özelliklerin çoğu Visual Studio tarafından desteklenen,, C#Python C++, F#JavaScript ve diğer dillere uygulanabilir (F# Düzenle ve devam et özelliğini desteklemez). F#ve JavaScript, **oto** penceresini desteklemez). Ekran görüntüleri Visual Basic.
...
2. **F5** tuşuna basın veya hata **ayıklamayı Başlat** ![düğmesine basın](../../debugger/media/dbg-tour-start-debugging.png "Hata Ayıklamayı Başlat"), uygulama başlar ve hata ayıklayıcı, kesme noktasını ayarladığınız kod satırına çalışır.
    ...
    ![Ayarlayın ve bir kesme noktası isabet](../visual-basic/media/get-started-hit-breakpoint-vb.png)
...
1. `name += letters[i]` bildiriminde duraklalarken, `letters` değişkeninin üzerine gelin ve varsayılan değeri, dizideki ilk öğenin değeri olan `"f"c`görürsünüz.
