---
description: Bu arabirim, oturum hata ayıklama Yöneticisi 'ne (SDM) bağlantı noktaları sağlar.
...
`CoCreateInstance`Bir bağlantı noktası tedarikçisine yapılan bir çağrı `GUID` Bu arabirimi döndürür (Bu, bu arabirimi elde etmenin tipik yoludur). Örnek:
