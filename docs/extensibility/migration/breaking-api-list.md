---
title: Visual Studio 2022'de YENI API değişiklikleri
description: Uzantıları 2022'ye değiştirirken mevcut VS uzantılarını derlemenin başarısız Visual Studio öğrenin.
ms.date: 06/08/2021
ms.topic: reference
...
Yeni değişiklikleri yönetmek için birlikte çalışma derlemelerinin dağıtımına yeni bir mekanizma sağlamayı planlıyoruz. Özellikle, Visual Studio 2022 ve ötesinde birçok ortak ortak çalışma arabiriminin tanımları ile tek bir birlikte çalışma derlemesi Visual Studio sağlariz. Bu derleme, birden çok birlikte çalışma derlemeden Visual Studio arabirimler için yönetilen tanımlar içerir. Yeni birlikte çalışma derlemesi, NuGet `Microsoft.VisualStudio.Interop` dağıtılır.

Ancak, Visual Studio olarak yerel bağlamlarda kullanılan ve az sayıda hataya neden olan değişikliklere sahip olan tüm bileşenler kendi birlikte çalışma derlemelerine sahip olmaya devam eder (örneğin, hata ayıklayıcı derlemesi VisualStudio.Debugger.Interop.dll olarak kabul edilecektir). Her durumda, derlemelere bugün olduğu gibi uygulamanıza başvurabilirsiniz.

Bu önemli bir değişikliktir ve bu yeni yaklaşımda yerleşik olarak API'leri kullanan uzantıların önceki birlikte çalışma derlemesini kullanan eski Visual Studio sürümleriyle uyumlu olmadığını ifade ediyor.
...
Bunun çok yaygın bir hata olmasını bekliyoruz. 2022'Visual Studio 64 bitlik bir işlem yapmak için, birlikte çalışma API'lerimizin bazılarının işaretçinin aslında işaretçi boyutlu bir değer kullanmak üzere 32 bit tamsayıya sığacak şekilde sığa tahminde bulunduğu şekilde düzeltilene gerek vardı.
...
Bu başvuruların bazen geçişli paket başvurusuyla geleblir ve bu nedenle kaldırmanın proje dosyanıza yapılan doğrudan başvurudan daha zor olacağını unutmayın. Böyle durumlarda, tüm doğrudan paket başvurularının 2022 SDK Visual Studio olduğundan emin olun. Kullanım dışı derlemeyi getirmekten sorumlu paket zincirini tanımlamak için *project.assets.json'a* başvurabilirsiniz. Geçişli paket başvurularını Visual Studio 2022 sürümüne güncelleştirmek, doğrudan başvuru olarak yüklemek kadar kolaydır.
...
hakkında bir hata `IAsyncServiceProvider` görüyorsanız, *yerel* kod (ikinci satır) için amaçlananı kullanıyor olabilir.
...
Varsayılan bağımsız değişkenin ne olduğu konusunda şüpheniz varsa dil hizmeti bağlamınızı Visual Studio 2022'den Visual Studio 2019'a değiştirerek varsayılan bağımsız değişkenin ne olduğunu görmek için eski birlikte çalışma derlemeleriyle Intellisense'i elde edin ve ardından kodunuzda açıkça ekleyin. Visual Studio 2019 için derlenmiş olduğunda iyi çalışmaya devam eder, ancak artık 2022 için Visual Studio olacaktır.
