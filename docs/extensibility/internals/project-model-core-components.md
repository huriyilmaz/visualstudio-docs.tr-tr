---
title: Proje Modeli Temel Bileşenleri | Microsoft Docs
...
### <a name="project-factory-object"></a>Project Factory nesnesi
...
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsUIHierarchy>|Kesme ve Yeniden Adlandırma gibi komutların yalnızca odak söz konusu olduğunda geçerli olduğu komutlar için `IOleCommandTarget` komutuna benzer şekilde komut yürütmeyi Çözüm Gezgini.|
|<xref:Microsoft.VisualStudio.OLE.Interop.IOleCommandTarget>|Bir proje hiyerarşisi için birincil komut hedef arabirimi olarak görev alır. Bu, nesneleri komut durumları veya durumları ve çalışan komutlar için sorgulamaya yönelik standart arabirimdir. Proje penceresine odaklanmadıyabilirsiniz.|
...
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsCfgProvider2>|Hata Ayıklama/Sürüm yapılandırmaları gibi proje yapılandırma nesnelerini yönetir. Oluşturma, dağıtma ve hata ayıklama işlemleri, proje yapılandırma nesneleri aracılığıyla eşgüdüm sağlar.|
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyDeleteHandler>|Hiyerarşi öğeleri için silme (yıkıcı) veya kaldırma (yıkıcı olmayan) seçeneklerini kontrol etmek için hiyerarşiler tarafından uygulanır. Arabiriminden arabirimde `IVsHierarchyDeleteHandler` Sorgu Arabirimini `IVsHierarchy` çağırma.|
...
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectStartupServices>|Projenizi diğer geliştiriciler tarafından genişletilebilir yapmak için uygulanan isteğe bağlı arabirim. Arabirimi, üçüncü taraf VSPackage'ın proje dosyanıza kalıcı olarak kaydederek projeniz her yüklensin, üçüncü taraf hizmet GUID'sini proje dosyanıza yüklemenizi ve bu GUID'i çağırmanızı `IVsProjectStartupServices` `QueryService` sağlar.|
...
|<xref:Microsoft.VisualStudio.Shell.Interop.SVsQueryEditQuerySave>|Tüm projeler ve düzenleyiciler, öğeleri kirli olmadan önce çağırarak veya kaydetmeden önce çağrılları gerekir.|
...
|<xref:Microsoft.VisualStudio.Shell.Interop.SVsUIShell>|Araç pencereleri veya belge pencereleri oluşturma ve numaralandırabilme ya da kullanıcıya hata bildirme gibi kullanıcı arabirimiyle ilgili temel IDE işlevleri sağlar.|
...
- [Derlemede Değil: Proje Türü Uygulamak için HierUtil7 Proje Sınıflarını Kullanma (C++)](/previous-versions/bb166212(v=vs.100))
