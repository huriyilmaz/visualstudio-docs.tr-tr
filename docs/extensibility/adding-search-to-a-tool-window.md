...
description: Arama kutusu, filtreleme ve ilerleme göstergesi dahil olmak üzere arama işlevlerini Visual Studio içindeki bir araç penceresine eklemeyi öğrenin.
...
ms.locfileid: "129439762"
---
# <a name="add-search-to-a-tool-window"></a>Araç penceresine arama ekleme
Uzantıda bir araç penceresi oluşturduğunuzda veya güncelleştirdiğinizde, Visual Studio başka bir yerde görüntülenen aynı arama işlevini ekleyebilirsiniz. Bu işlevsellik aşağıdaki özellikleri içerir:
...
6. menü çubuğunda,   >  **diğer Windows**  >  **test araması**' nı seçin.
...
3. Visual Studio deneysel örneğinde **testsearch**' u açın.
...
3. Aşağıdaki adımları gerçekleştirerek arama uygulamanızı test edin:
    ...
    2. Visual Studio deneysel örneğinde, araç penceresini yeniden açın, arama penceresinde bir arama metni girin ve **enter**' a tıklayın.
...
## <a name="to-add-a-search-filter"></a>Arama filtresi eklemek için
 Kullanıcıların arama hedefleri kümesini iyileştirmesine izin veren arama filtreleri ekleyebilirsiniz. Örneğin, dosya Gezgini 'ndeki dosyaları en son değiştirildiği tarih ve bunların dosya adı uzantılarına göre filtreleyebilirsiniz. Bu kılavuzda, yalnızca çift satırlar için bir filtre ekleyeceksiniz. Kullanıcı bu filtreyi seçerken, arama ana bilgisayarı belirttiğiniz dizeleri arama sorgusuna ekler. Daha sonra bu dizeleri arama yönteminizin içinde tanımlayabilir ve arama hedeflerini buna göre filtreleyebilirsiniz.

1. *TestSearch.cs dosyasında* sınıfına aşağıdaki kodu `TestSearch` ekleyin. Kod, arama `SearchFiltersEnum` sonuçlarını yalnızca <xref:Microsoft.VisualStudio.PlatformUI.WindowSearchSimpleFilter> satırlar bile görünecek şekilde filtrelemeyi belirten bir ekleyerek uygulanır.
    ...
    ```
     ...
     Artık arama denetimi arama filtresini `Search even lines only` görüntüler. Kullanıcı filtreyi seçerse, dize `lines:"even"` arama kutusunda görünür. Diğer arama ölçütleri filtreyle aynı anda görünebilir. Arama dizeleri filtreden önce, filtreden sonra veya her ikisi birden görünebilir.

2. *TestSearch.cs* dosyasında, sınıfındaki `TestSearchTask` sınıfına aşağıdaki yöntemleri `TestSearch` ekleyin. Bu yöntemler, `OnStartSearch` bir sonraki adımda değiştirerek yöntemini destekler.
...
3. sınıfında `TestSearchTask` yöntemini aşağıdaki `OnStartSearch` kodla güncelleştirin. Bu değişiklik, filtreyi desteklemek için kodu günceller.
...
5. Projeyi derleme ve hata ayıklamayı başlatma. Deneysel Visual Studio araç penceresini açın ve arama denetiminde Aşağı okunu seçin.

     **Büyük/küçük harf** eşleşmesi onay kutusu **ve Yalnızca arama satırları** filtresi görüntülenir.

6. Filtreyi seçin.

     Arama kutusu **satırlar içerir:"even"** ve aşağıdaki sonuçlar görüntülenir:
...
7. Arama `lines:"even"` kutusundan silin, Durumu **eşle onay** kutusunu seçin ve arama `g` kutusuna girin.
...
8. Arama kutusunun sağ tarafındaki X'i seçin.
     ...
     Arama temiz ve özgün içerikler görüntülenir. Ancak, Eşleşme **durumu** onay kutusu hala seçilidir.
