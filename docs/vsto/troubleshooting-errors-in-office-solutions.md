...
## <a name="creating"></a>Projeleri oluşturun, yükseltin ve açın
...
## <a name="designers"></a>Tasarımcıları kullanma
...
## <a name="code"></a>Kod Yaz
 Office projelerinde kod yazdığınızda aşağıdaki hatalarla karşılaşabilirsiniz.

### <a name="some-events-of-office-objects-are-not-accessible-when-using-c"></a>C\# kullanılırken Office nesnelerinin bazı olaylarına erişilemez
 ...
 "' Microsoft. Office. Interop. Excel. _Application. NewWorkbook ' ile ' Microsoft. Office. Interop. Excel. AppEvents_Event. NewWorkbook '" arasında belirsizlik
 ...
 Olayları olan Office PIA türleri iki arabirim uygular: tüm özellikleri ve yöntemleri içeren bir çekirdek arabirim ve nesne tarafından açığa çıkarılan olayları içeren bir olay arabirimi. Bu olay arabirimleri, <xref:Microsoft.Office.Interop.Excel.AppEvents_Event> ve <xref:Microsoft.Office.Interop.Word.ApplicationEvents2_Event>gibi adlandırma kuralı *ObjectName*olayları*n*_olayını kullanır. Bir nesne üzerinde bulmayı düşündüğünüz bir olaya erişemiyorsanız, nesneyi olay arabirimine atayın.

 Örneğin, <xref:Microsoft.Office.Interop.Excel.Application> nesnelerinin <xref:Microsoft.Office.Interop.Excel.AppEvents_Event.NewWorkbook> bir olayı ve bir <xref:Microsoft.Office.Interop.Excel._Application.NewWorkbook%2A> özelliği vardır. <xref:Microsoft.Office.Interop.Excel.AppEvents_Event.NewWorkbook> olayını işlemek için <xref:Microsoft.Office.Interop.Excel.Application> <xref:Microsoft.Office.Interop.Excel.AppEvents_Event> arabirimine atayın. Aşağıdaki kod örneği, Excel için belge düzeyindeki bir projede bunun nasıl yapılacağını göstermektedir.
...
### <a name="cannot-reference-office-pia-classes-in-projects-that-target-the-includenet_v40_shortsharepointincludesnet-v40-short-mdmd-or-the-includenet_v45vstoincludesnet-v45-mdmd"></a>[!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] veya [!INCLUDE[net_v45](../vsto/includes/net-v45-md.md)] hedefleyen projelerde Office PIA sınıflarına başvuramaz
 [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] veya [!INCLUDE[net_v45](../vsto/includes/net-v45-md.md)]hedefleyen projelerde, bir Office PIA ' de tanımlanan bir sınıfa başvuruda bulunan kod varsayılan olarak derlenmez. PIA 'ler içindeki sınıflar, <xref:Microsoft.Office.Interop.Word.DocumentClass> ve <xref:Microsoft.Office.Interop.Excel.WorkbookClass>gibi adlandırma kuralı *ObjectName*sınıfını kullanır. Örneğin, bir Word VSTO eklenti projesinden aşağıdaki kod derlenmeyecektir.
...
- Visual C#: "' Microsoft. Office. Interop. Word. DocumentClass ' birlikte çalışma türü Katıştırılamaz. Bunun yerine uygulanabilir arabirimi kullanın. "
  ...
  Bu hatayı çözmek için, kodu bunun yerine karşılık gelen arabirime başvuracak şekilde değiştirin. Örneğin, bir <xref:Microsoft.Office.Interop.Word.DocumentClass> nesnesine başvurmak yerine <xref:Microsoft.Office.Interop.Word.Document> arabiriminin bir örneğine başvurun.
...
## <a name="building"></a>Yapı projeleri
...
### <a name="compiler-errors-occur-after-a-namedrange-control-is-deleted"></a>NamedRange denetimi silindikten sonra derleyici hataları oluşur
 Tasarımcıda etkin çalışma sayfası olmayan bir çalışma sayfasından bir <xref:Microsoft.Office.Tools.Excel.NamedRange> denetimini silerseniz, otomatik olarak oluşturulan kod projenizden kaldırılmayabilir ve derleyici hataları oluşabilir. Kodun kaldırıldığından emin olmak için, denetimi silmeden önce etkin çalışma sayfası yapmak üzere <xref:Microsoft.Office.Tools.Excel.NamedRange> denetimi içeren çalışma sayfasını her zaman seçmeniz gerekir. Denetimi sildiğinizde otomatik olarak oluşturulan kod silinmemişse, tasarımcı 'nın çalışma sayfasını etkinleştirerek ve çalışma sayfasının değiştirilmiş olarak işaretlendiğinden bir değişiklik yaparak kodu silmesine neden olabilirsiniz. Projeyi yeniden oluşturduğunuzda kod kaldırılır.

## <a name="debugging"></a>Hata ayıklama projeleri
