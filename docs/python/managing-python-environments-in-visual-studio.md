...
Windows Visual Studio da ortamları yönetmek ve yeni projeler için varsayılan olarak birini seçmek için bu makalede açıklandığı gibi **Python** Ortamları penceresini kullanırsınız. Ortamların diğer yönleri aşağıdaki makalelerde bulunabilir:
...
Python'da yeniysiniz Visual Studio aşağıdaki makaleler de genel arka plan bilgileri sağlar:
...
> [!Note]
> Klasör olarak açılan Python kodu ortamlarını Dosya Klasör Aç **komutunu**  >  **kullanarak**  >  **yönetebilirsiniz.** Python araç çubuğu, algılanan tüm ortamlar arasında geçiş yapmak ve yeni bir ortam eklemek için kullanılabilir. Ortam bilgileri Çalışma Alanı PythonSettings.jsklasöründeki dosyada depolanır.
::: moniker-end
...
> [!Note]
> Bu Visual Studio system-site-packages seçeneğine uygun olsa da, bunu sistem içinde değiştirmek için bir yol Visual Studio.
...
Küresel bir ortamda çalışmak, çalışmaya başlamanın kolay bir yolu olsa da, bu ortam zaman içinde farklı projeler için yüklemiş olduğunuz birçok farklı paketle karmaşık hale gelir. Bu tür dağınıklıklar, bir uygulamayı bilinen sürümlere sahip belirli bir paket kümesine karşı ayrıntılı bir şekilde test etmek için zorlaştırır. Bu, derleme sunucusunda veya web sunucusunda tam olarak ayarlayışınız ortam t türlerindedir. İki proje uyumsuz paketler veya aynı paketin farklı sürümlerine ihtiyaç olduğunda da çakışmalar oluşabilir.
...
Visual Studio proje için sanal ortam oluşturmaya yönelik doğrudan destek sağlar. Örneğin, *requirements.txt* içeren bir proje açar veya bu dosyayı içeren bir şablondan proje oluşturursanız, Visual Studio otomatik olarak bir sanal ortam oluşturmanızı ve bu bağımlılıkları yüklemeni ister.
...
1. Bu Visual Studio proje için Bir ortam seçin konusunda açıklandığı gibi bir proje için conda [ortamını etkinleştirerek diğer ortamlar gibi etkinleştirabilirsiniz.](selecting-a-python-environment-for-a-project.md)
...
Bir Visual Studio kayıt defteri girdilerini bulursanız ancak yorumlayıcının yolu geçersizse **Python Ortamları** penceresinde adı bir yazı tipiyle birlikte gösterir:
...
1. InstallPath düğümü altındaki **değerleri inceleme:**
    ...
    - Geçersiz ayarlar **HKEY_CURRENT_USER\SOFTWARE\Python** ayarları geçersiz **kılacakHKEY_LOCAL_MACHINE\SOFTWARE\Python**
