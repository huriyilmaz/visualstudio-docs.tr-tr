...
Dosya Yeni Dosyasını kullanarak şablondan proje  >  **Project.**  >   Web projelerine yönelik şablonları görmek için iletişim kutusunun sol tarafındaki **Python**  >  **Web'i** seçin. Ardından proje ve çözüm için adlar sağlayarak istediğiniz şablonu seçin, çözüm dizini ve Git deposu için seçenekler belirleyin ve Tamam'ı **seçin.**

![Web uygulamaları için yeni proje iletişim kutusu](media/projects-new-project-dialog-web.png)

::: moniker range="<=vs-2017"

Daha önce **Project** web uygulaması şablonu, kod olmayan ve Python projesi Visual Studio varsayımları olmayan boş bir Visual Studio projesi sağlar.
...
Tüm **Boş \<framework> Web Project** şablonları, az veya çok az ortak kod ve birrequirements.txtdosyasında bildirilen gerekli *bağımlılıklarla bir proje* oluşturur.

| Şablon | Description |
| --- | --- |
| **Boş Bottle Web Project** | çok kısa bir satır *içi app.py* kullanarak bir giriş sayfası ve yankı oluşturan bir sayfayla birlikte, uygulamada çok az `/` uygulama `/hello/<name>` `<name>` oluşturur. |
...
**Polls \<framework> Web Project** şablonları, kullanıcıların farklı yoklama sorularına oy verecekleri bir başlangıç web uygulaması oluşturur. Her uygulama, yoklamaları ve kullanıcı yanıtlarını yönetmek için bir veritabanı kullanmak üzere **Web** projesi şablonlarının yapısı üzerine kurulur. Uygulamalar uygun veri modellerini ve *samples.json* dosyasından yoklama yüken özel bir uygulama sayfasını (/seed) içerir.

| Şablon | Description |
| --- | --- |
| **Polls Bottle Web Project** | Ortam değişkeni kullanılarak yapılandırılan, bellek içinde veritabanı, MongoDB veya Azure Tablo Depolama çalıştırabilecek bir `REPOSITORY_NAME` uygulama oluşturur. Veri modelleri ve veri deposu kodu *models* klasöründe bulunur ve *settings.py* dosyası hangi veri deposunın kullanlı olduğunu belirlemek için kod içerir. |
| **Django Web sitesini yoklar Project** | Bir Django projesi ve üç sayfalı bir Django uygulaması ve bir SQLite veritabanı oluşturur. Kimliği doğrulanmış bir yöneticinin yoklamaları oluşturmasına ve yönetmesine izin vermek için Django yönetim arabiriminde özelleştirmeler içerir. Daha fazla bilgi için [bkz. Django şablonları ve](python-django-web-application-project-template.md) [Learn Django 6. Adım.](learn-django-in-visual-studio-step-06-polls-django-web-project-template.md) |
| **Flask/Jade Web sitesini yoklar Project** | **Polls Flask Web** Project şablonuyla aynı uygulamayı oluşturur, ancak Jinja şablon oluşturma altyapısı için Jade uzantısını kullanır. |
::: moniker-end
...
Kaynak denetimi kullanıyorsanız, genellikle sanal ortam klasörünü atlarsınız çünkü bu ortam yalnızca *requirements.txt.* Klasörü dışlamanın en iyi yolu,  önce yukarıda gösterilen istemde kendi yükleyeceğim öğesini seçmek ve ardından sanal ortamı oluşturmadan önce otomatik işlemeyi devre dışı bırakmaktır. Ayrıntılar için bkz. [Learn Django Öğreticisi - 1-2 ve 1-3.](learn-django-in-visual-studio-step-01-project-and-solution.md#step-1-2-examine-the-git-controls-and-publish-to-a-remote-repository) Adımlar ve [Learn Flask Öğreticisi - 1-2 ve 1-3. Adımlar.](learn-flask-visual-studio-step-01-project-solution.md#step-1-2-examine-the-git-controls-and-publish-to-a-remote-repository)

Microsoft Azure App Service dağıtım Microsoft Azure App Service python sürümünü site uzantısı olarak [seçin ve](./managing-python-on-azure-app-service.md?view=vs-2019&preserve-view=true) paketleri el ile yükleyin. Ayrıca, Azure App Service **bir** *requirements.txt* dosyasından paket yüklemez, Visual Studio yapılandırma [ayrıntılarını](managing-python-on-azure-app-service.md)aka.ms/PythonOnAppService.
...
- **Arama Yolları,** **Betik Bağımsız Değişkenleri,** **Yorumlayıcı** Bağımsız Değişkenleri ve **Yorumlayıcı Yolu:** Bu seçenekler normal hata ayıklama [ile aynıdır.](debugging-python-in-visual-studio.md)
- **BAŞLATMA** URL'si: Tarayıcınızda açılan URL'yi belirtir. Varsayılan olarak değeri `localhost` kullanılır.
...
Sunucu Komutunu Çalıştır ve  **Sunucu Komut** Gruplarında Hata Ayıkla gruplarında (görüntüde gösterilenlerin ikincisi aşağıdadır) özellikler, web sunucusunun nasıl başlatıla olduğunu belirler. Birçok çerçeve, geçerli projenin dışında bir betik kullanımını gerektirmesi nedeniyle, betik burada ya da başlangıç modülünün adı bir parametre olarak geçiriliyor olabilir.

- **Komut:** Python betiği (*\* .py* dosyası), modül adı (içinde olduğu gibi) veya tek bir kod `python.exe -m module_name` satırı (örneğin, ) `python.exe -c "code"` olabilir. Açılan liste değeri, bu türlerden hangilerinin amaçlanan olduğunu gösterir.
...
Herhangi bir proje özelliği veya ortam değişkeni, belirli bir MSBuild ile belirtilebilir, örneğin: `$(StartupFile) --port $(SERVER_PORT)` .
`$(StartupFile)` , başlangıç dosyasının göreli yoludur `{StartupModule}` ve başlangıç dosyasının içeri aktarılabilir adıdır. `$(SERVER_HOST)`ve , Başlatma URL'si ve Bağlantı Noktası Numarası özellikleri tarafından otomatik olarak veya Ortam özelliği tarafından ayarlanmış `$(SERVER_PORT)` normal ortam **değişkenleridir.**  
