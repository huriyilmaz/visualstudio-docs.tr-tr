...
Visual Studio ve TypeScript ASP.NET Core için bu öğreticide basit bir web uygulaması oluşturacak, bazı TypeScript kodu ekp uygulamayı çalıştırabilirsiniz.

::: moniker range=">=vs-2022"

Visual Studio 2022'den başlayarak, ASP.NET Core ile Angular veya Vue kullanmak için TypeScript ile bir ASP.NET Core uygulaması oluşturmak için ASP.NET Core Tek Sayfalı Uygulama (SPA) şablonlarını kullanmanız önerilir. Daha fazla bilgi için bkz. Visual Studio veya Vue [için Angular](../javascript/tutorial-asp-net-core-with-angular.md) [öğreticileri.](../javascript/tutorial-asp-net-core-with-vue.md)
::: moniker-end
...
## <a name="create-a-new-aspnet-core-mvc-project"></a>MVC projesi ASP.NET Core oluşturma
...
1. Projenize [dosya eklemek için npm](../javascript/npm-package-management.md#aspnet-core-projects) paket yönetimi `package.json` yönergelerini izleyin. Bu, projenize npm desteği ekler.
   ...
   >[!NOTE]
   > Daha ASP.NET Core, istemci tarafı JavaScript ve CSS dosyalarını yüklemek için npm yerine [Kitaplık](/aspnet/core/client-side/libman/) Yöneticisi'ni veya yarn'i de kullanabilirsiniz.

1. Bu örnekte, projenize jQuery için bir TypeScript tanım dosyası ekleyin. *Package.json dosyanıza aşağıdakileri* dahil etmek.
...
1. uygulama paketi Çözüm Gezgini npm düğümüne sağ tıklayın ve Paketleri Geri **Yükle'yi seçin.**
   ...
   >[!NOTE]
   > Bazı senaryolarda, Çözüm Gezgini burada açıklanan bilinen bir sorun nedeniyle npm paketinin *package.json* ile eşitlenemediği [belirtebildi.](https://github.com/aspnet/Tooling/issues/479) Örneğin, paket yüklenirken yüklenmemiş gibi görünebilir. Çoğu durumda, *package.json'Çözüm Gezgini* silerek, Visual Studio başlatarak ve bu makalede daha önce açıklandığı gibi *package.json* dosyasını yeniden ekleyerek bu dosyayı güncelleştirebilirsiniz.
