...
ClickOnce dağıtım bildirimi oluşturur. ClickOnce dağıtım bildirimi, dağıtım için benzersiz bir kimlik tanımlayarak, uygulama güncelleştirme ayarlarını ve güncelleştirme konumlarını belirterek, kurulum veya çevrimiçi mod gibi dağıtım niteliklerini tanımlayarak uygulamanın dağıtımını açıklar ve karşılık gelen ClickOnce uygulaması bildirimini belirtir.
...
| `MaxTargetPath` | İsteğe bağlı `String` parametresi.<br /><br /> ClickOnce uygulama dağıtımında bir dosya yolunun izin verilen maksimum uzunluğunu belirtir. Bu parametre belirtilmişse, uygulamadaki her dosya yolunun uzunluğu bu sınıra karşı denetlenir. Sınırı aşan tüm öğeler, derleme uyarısına neden olur. Bu giriş belirtilmemişse veya sıfırsa, hiçbir denetim yapılmaz. |
...
| `Platform` | İsteğe bağlı `String` parametresi.<br /><br /> Uygulamanın hedef platformunu belirtir. Bu parametre aşağıdaki değerlere sahip olabilir:<br /><br /> -   `AnyCPU`<br />-   `x86`<br />-   `x64`<br />-   `Itanium`<br /><br /> Varsayılan değer: `AnyCPU`. |
