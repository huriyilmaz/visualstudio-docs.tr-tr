...
| BaseOutputPath | Çıkış dosyası için temel yolu belirtir. Ayarlanırsa, MSBuild kullanır `OutputPath = $(BaseOutputPath)\$(Configuration)\` . Örnek sözdizimi:`<BaseOutputPath>c:\xyz\bin\</BaseOutputPath>` |
| BuildInParallel | Multi-proc MSBuild kullanılırken proje başvurularının paralel olarak oluşturulup temizlenmediğini belirten bir Boolean değer. Varsayılan değer `true` , bu, sistemin birden çok çekirdeği veya işlemciyi varsa, projenin paralel olarak derlenme anlamına gelir. |
| BuildProjectReferences | Proje başvurularının MSBuild tarafından oluşturulup oluşturulmayacağını gösteren bir Boolean değer. `false`Projenizi Visual Studio tümleşik geliştirme ortamında (IDE) oluşturuyorsanız, `true` Aksi takdirde olarak ayarlayın. `-p:BuildProjectReferences=false`, başvurulan projelerin güncel olup olmadığını kontrol etmekten kaçınmak için komut satırında belirtilebilir. |
...
| CscToolPath | *CSC. exe*' nin, C# derleyicisinin yolu. |
...
| Belirlenimci | Derleyicinin özdeş girdiler için özdeş derlemeler üretmesi gerekip gerekmediğini belirten bir Boolean değer. Bu parametre, `/deterministic` *Vbc. exe* ve *CSC. exe* derleyicilerinin anahtarına karşılık gelir. |
...
| FrameworkPathOverride | *Mscorlib. dll* ve *Microsoft. VisualBasic. dll*dosyasının konumunu belirtir. Bu parametre, `/sdkpath` *Vbc. exe* derleyicisinin anahtarıyla eşdeğerdir. |
...
| GenerateFullPaths | Þ [-Fullpaths](/dotnet/csharp/language-reference/compiler-options/fullpaths-compiler-option) derleyici seçeneğini kullanarak çıkışta dosya adları için tam yollar oluşturun. |
...
| IntermediateOutputPath | Yol belirtilmemişse tam ara çıkış yolu öğesinden türetilir `BaseIntermediateOutputPath` . Örneğin, *\obj\debug \\ *. |
...
| Msbuildprojeclarsionspath | Proje uzantılarının bulunduğu yolu belirtir. Varsayılan olarak, bu değeri ile aynı şekilde alır `BaseIntermediateOutputPath` . |
...
| OptionCompare | Dize karşılaştırmalarının nasıl yapılacağını belirtir. Geçerli değerler şunlardır "binary" veya "Text." Bu özellik `/optioncompare` *Vbc. exe*' nin derleyici anahtarıyla eşdeğerdir. |
...
| PathMap | Derleyici tarafından giden kaynak yol adlarına fiziksel yolların nasıl eşleneceğini belirtir. Bu özellik, `/pathmap` *CSC. exe* derleyicisinin anahtarıyla eşdeğerdir. |
...
| ProduceOnlyReferenceAssembly | Derleyiciye derlenen kod yerine yalnızca bir başvuru derlemesi yaymasını bildiren bir Boole değeri. İle birlikte kullanılamaz `ProduceReferenceAssembly` .  Bu özellik `/refonly` *Vbc. exe* ve *CSC. exe* derleyicilerinin anahtarına karşılık gelir. |
| ProduceReferenceAssembly | `true`Geçerli derleme için [başvuru derlemelerinin](/dotnet/standard/assembly/reference-assemblies) üretimini sağlamak üzere ayarlandığında ayarlanmış bir Boole değeri. `Deterministic``true`Bu özellik kullanılırken olmalıdır. Bu özellik `/refout` *Vbc. exe* ve *CSC. exe* derleyicilerinin anahtarına karşılık gelir. |
...
| Skipınvalidconfigurations | Ne zaman `true` , geçersiz platform ve yapılandırma birleşimleri üzerinde bir uyarı oluştur, ancak derlemeyi başarısız yap; ne zaman `false` veya tanımsız (varsayılan), bir hata oluşturur. |
...
| UseHostCompilerIfAvailable | Varsa, derleme görevinin, varsa `true` işlem içi derleyici nesnesini kullanmasına neden olan bir Boole parametresi. Bu parametre yalnızca Visual Studio tarafından kullanılır. |
...
| Vbcayrıntı düzeyi | Visual Basic derleyicisinin çıkışının ayrıntı düzeyini belirtir. Geçerli değerler şunlardır "sessiz," "normal" (varsayılan değer) veya "ayrıntılıdır." |
