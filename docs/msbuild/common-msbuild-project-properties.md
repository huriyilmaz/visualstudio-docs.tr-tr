...
| BaseOutputPath | Çıkış dosyası için temel yolu belirtir. Ayarlanırsa, MSBuild `OutputPath = $(BaseOutputPath)\$(Configuration)\`kullanır. Örnek sözdizimi: `<BaseOutputPath>c:\xyz\bin\</BaseOutputPath>` |
| BuildInParallel | Multi-proc MSBuild kullanılırken proje başvurularının paralel olarak oluşturulup temizlenmediğini belirten bir Boolean değer. Varsayılan değer `true`, bu, sistemin birden çok çekirdeği veya işlemciyi varsa, bu projelerin paralel olarak derlenme anlamına gelir. |
| BuildProjectReferences | Proje başvurularının MSBuild tarafından oluşturulup oluşturulmayacağını gösteren bir Boolean değer. Projenizi Visual Studio tümleşik geliştirme ortamında (IDE) oluşturuyorsanız, aksi takdirde `true` `false` olarak ayarlayın. başvurulan projelerin güncel olup olmadığını kontrol etmekten kaçınmak için, komut satırında `-p:BuildProjectReferences=false` belirtilebilir. |
...
| CscToolPath | *CSC. exe*' nin yolu, C# derleyici. |
...
| DisableFastUpToDateCheck | Yalnızca Visual Studio için geçerli olan bir Boole değeri. Visual Studio derleme Yöneticisi, bir projenin güncel olması için yeniden oluşturulması gerekip gerekmediğini öğrenmek için FastUpToDateCheck adlı bir işlem kullanır. Bu işlem, bunu anlamak için MSBuild kullanmaktan daha hızlıdır. DisableFastUpToDateCheck özelliğinin `true` olarak ayarlanması, Visual Studio derleme Yöneticisi 'ni atlamanızı ve projenin güncel olup olmadığını belirlemede MSBuild 'i kullanmaya zorlamanızı sağlar. |
...
| UseHostCompilerIfAvailable | `true`, derleme görevinin varsa işlem içi derleyici nesnesini kullanmasına neden olan bir Boole parametresi. Bu parametre yalnızca Visual Studio tarafından kullanılır. |
