 ...
 *MSBuild. exe* ' yi, daha karmaşık derlemeler gerçekleştirmek için kullanabilirsiniz. Örneğin, bir çözümde belirli projelerin belirli hedeflerini oluşturmak için kullanabilirsiniz. Aşağıdaki örnek, projeyi yeniden oluşturur `NotInSolutionFolder` ve `InSolutionFolder` *newfolder* çözüm klasöründeki projeyi temizler.
